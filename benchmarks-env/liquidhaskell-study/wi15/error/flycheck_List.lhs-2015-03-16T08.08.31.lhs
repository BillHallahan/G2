...
Fix the specification for `map`m such that the assertion in `prop_map`
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {xs:List a | 1 <= size xs} -> a @-}
...
{-@ measure sizeN :: List (List a) -> Int
     sizeN (Emp) = 0
     sizeN ((:+:) x xs) = size x + sizeN xs     
@-}

{-@ listHead :: List a -> a @-}
listHead (x :+: xs) = x
{-@ listTail :: xs:List a -> {v: List a | size v = (size xs) - 1} @-}
listTail (x :+: xs) = xs
{-@ unravel :: List a -> a @-}
unravel xs = listHead xs :+: unravel $ listTail xs

{-@ concat :: xs:List (List a) -> ListN a (sizeN xs) @-}
concat Emp = Emp
concat (x :+: xs) = unravel x :+: concat xs
