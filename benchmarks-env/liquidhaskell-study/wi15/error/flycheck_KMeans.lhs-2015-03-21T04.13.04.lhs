...
\begin{code}
{-@ distance :: n:Nat -> p1:Point -> {p2:Point | size p1 == 0 <=> size p2 == 0} -> Double @-}
...
\begin{code}
myDists k n centers p dists = 

{-@ nearest :: k:Nat -> n:Nat -> CenteringKN k n -> PointN n -> CenterK k @-}
nearest k n centers p = distance n p (minKeyMap centers)
...
\begin{code}
{-@ mergeCluster :: n:Nat -> c1:Cluster -> {c2:Cluster | size (snd c1) == 0 <=> size (snd c2) == 0} -> Cluster @-}
