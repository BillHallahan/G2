...
\begin{code}
{-@ distance :: n:Nat -> xs:Point -> {ys:Point | size ys = size xs} -> Double @-}
...
nearest k n centers p = fst $ minimumBy (\(_, x1) (_, x2) -> compare (distance n x1 p) (distance n x2 p)) (M.toList centers)
\end{code}
...
\begin{code}
{-@ mergeCluster :: n:Nat -> Cluster _ xP -> Cluster _ ({yP:List Double | size yP = size xP}) -> Cluster _ ({zP:List Double | size zP = size yP}) @-}
