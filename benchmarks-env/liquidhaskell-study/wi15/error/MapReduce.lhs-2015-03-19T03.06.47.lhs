...
\begin{code}
{-@ group :: (Ord k) => {v:List (k, v)| size v > 0} -> M.Map k (List v) @-}
...
{-@ measure msize :: Map k v -> Int @-}
--{-@ assume msize :: m:Map k v -> {i:Int | }@-}
{-@ type MapN k v N = {m:M.Map k v | msize m == N} @-}

{-@ toList :: m:(M.Map k v) -> {v:List (k, v) | M.size m == size v}  @-}
