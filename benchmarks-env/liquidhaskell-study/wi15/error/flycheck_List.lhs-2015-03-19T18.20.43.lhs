...
\begin{code}
{-@ length :: ls:List a -> {v:Int | size ls = v} @-}
length            :: List a -> Int
length Emp        = 0
...
{-@ measure concatsize   :: List (List a) -> Int
    concatsize xs = 1 + size (concat xs)
  @-}
{-@ concat :: xs:List (List a) -> ListN a (concatsize xs) @-}
concat = foldll append Emp
...
foldll _  b Emp = b
foldll op b (x :+: xs) = foldll op (op b x) xs

append :: List a -> List a -> List a
append xs Emp = xs
append xs (y :+: ys) = append (y :+: xs) ys
\end{code}
