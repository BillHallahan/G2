...
{-@ type NonEmptyList a = {v:List a | size v > 0} @-}

{-@ mapReduce :: (a -> NonEmptyList (k, v)) -> (v -> v-> v) -> NonEmptyList a -> M.Map k v @-}
...
\begin{code}
{-@ map :: (a -> NonEmptyList (k,v)) -> NonEmptyList a -> NonEmptyList (NonEmptyList (k, v)) @-}
{-@ expand  :: (a -> NonEmptyList (k, v)) -> NonEmptyList a -> NonEmptyList (k, v) @-}
...
\begin{code}
{-@ group :: (Ord k) => NonEmptyList (k, v) -> M.Map k (NonEmptyList v) @-}
...
\begin{code}
{-@ collapse  :: (v -> v -> v) -> M.Map k (NonEmptyList v) -> M.Map k v @-}
