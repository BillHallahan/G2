...
\begin{code}
{-@ map :: (a -> b) -> va:List a -> {vb:List b | size vb = size va} @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> va:List a -> {vb:List b | size va == size vb} -> {vc:List c | size va == size vc} @-}
...
{-@ measure count :: (List (List a)) -> Int
   count (Emp) = 0
   count (va `:+:` vb) = (size va) + (count vb)
@-}

{-@ concat :: vb:(List (List a)) -> {va:List a| size va == count vb} @-}
concat Emp = Emp
concat (Emp :+: vb) = concat vb
concat (va :+: vb) = va
concat ((x :+: va) :+: vb) = x :+: (concat (va :+: vb))
