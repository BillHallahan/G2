...
\begin{code}
{-@ distance :: n:Nat -> xs:Point -> {ys:Point | size xs = size ys} -> Double @-}
...
{-@ measure cludim :: Cluster -> Int
    cludim (n, p)  = size p
  @-}
{-@ mergeCluster :: n:Nat -> a:ClusterP -> ClusterN (cludim a) -> ClusterN (cludim a) @-}
...
{-@ kmeans :: Nat -> k:Nat -> n:Nat ->
              {a:List (PointN n) | size a > 0} -> CenteringKN k n -> CenteringKN k n
  @-}
...
kmeans1 k n ps cs = normalize newClusters ps
  where
    {-@ normalize :: M.Map a ClusterP -> M.Map a Point @-}
    normalize      :: M.Map a Cluster -> M.Map a Point
    normalize      = M.map (\(sz, p) -> centroid n p sz)
    newClusters    = mapReduce fm fr
    {-@ fm :: a:Point -> List (Center, (ClusterN (size a)))  @-}
    fm p           = singleton (nearest k n cs p, (1 :: Int, p))
    {-@ fr :: a:ClusterP -> ClusterN (cludim a) -> ClusterN (cludim a) @-}
