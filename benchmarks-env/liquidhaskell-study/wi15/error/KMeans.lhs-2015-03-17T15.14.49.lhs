...
\begin{code}
{-@ distance :: n:Nat -> x:Point -> y:PointN (size x) -> Double @-}
...
-- newMap k n centers p = M.map (\px -> distance n p px) centers
nearest k n centers p = minKeyMap (M.map (\px -> distance n p px) centers)
-- nearest k n centers p = minKeyMap (map (\px -> distance n p px) centers)
\end{code}
...
\begin{code}
{-@ mergeCluster :: n:Nat -> (n1, p1):Cluster -> {(n2, p2):Cluster|size p1 == size p2} -> Cluster @-}
