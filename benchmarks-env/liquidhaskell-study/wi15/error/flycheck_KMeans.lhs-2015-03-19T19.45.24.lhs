...
\begin{code}
{-@ distance :: xs:Point -> PointN (size xs) -> Double @-} ---minimum of 
distance px py = sqrt $ foldr (+) 0 $ zipWith (\x y -> (x-y)^2) px py
\end{code}
...
{-@ centerToList :: CenteringKN k n -> [(k, PointN n)] @-}
centerToList centers = M.toList $ centers

---map body (centerToList centers)
distOnList :: [(k, PointN n)] -> PointN n -> [(k, double)]
distOnList xs p = map body (k,pList):xs
             where body = zipWith (distance) pList p 
\end{code}
