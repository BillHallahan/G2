...
\begin{code}
{-@ replicate :: n:Int -> a -> {v:List a | n = size v} @-}
replicate 0 _ = Emp
...
\begin{code}
{-@ map :: (a -> b) -> xs:List a -> {ys:List b | size xs = size ys} @-}
map f Emp        = Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {xs:List a | 0 < size xs} -> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> xs:List a -> {ys:List b | size xs = size ys} -> {zs:List c | size xs = size zs}@-}
...
\begin{code}
{-@ invariant (addList xs:(List a) ys:(List a)) -> {vs:List a | size vs == size xs + size ys} @-}

{-@ addList :: xs:List a -> ys:List a -> {zs:List a | size xs + size ys = size zs} @-}
addList Emp ys = ys
addList xs  ys = foldr (:+:) ys xs


{-@ measure concat_size :: List (List a) -> Int
	concat_size (Emp) = 0
	concat_size ((:+:) xs xss) = size xs + concat_size xss
@-}

{-@ concat :: xss:(List (List a)) -> {ys:List a | concat_size xss = size ys} @-}
concat Emp = Emp
concat (xs :+: xss) = addList xs (concat xss)
--concat (xs :+: xss) = foldr (:+:) (concat xss) xs
