...
\begin{code}
{-@ type ValidList a = {v:List a | size v >= 0} @-}
{-@ zipWith :: (a -> b -> c) -> x:ValidList a -> {y:ValidList b | size x = size y} -> {v:List c | size v = size x && size v = size y} @-}
...
\begin{code}
{-@ merge :: x:List a -> y:List a -> {v:List a | size v = size x + size y} @-}
merge Emp Emp = Emp
merge Emp b   = b
merge a   Emp = a
merge (x:+:xs) b = x :+: merge xs b

{-@ measure inner_size  :: List(List a) -> Int
            inner_size Emp = 0
            inner_size ((:+:) x xs)  = (size x) + inner_size xs
@-}

{-@ concat :: x:List(List a) -> {v:List a | size v = inner_size x} @-}
concat Emp = Emp
concat (x :+: xs) = merge x (concat xs)
