...
\begin{code}
{-@ measure totalSize    :: List(List a) -> Int
    totalSize Emp        = 0
    totalSize ((:+:) x xs) = (size x) + (totalSize xs)
@-}

{-@ subConcat :: l1:(List a) -> l2:(List a) -> {l3:List a | size l3 == (size l1) + (size l2)} @-}

subConcat Emp l        = l
subConcat (x :+: xs) l = x :+: (subConcat xs l)

{-@ concat :: input:(List (List a)) -> {output:List a | size output == totalSize input} @-}

concat Emp l      = l
concat (l :+: ls) = subConcat l (concat ls)
