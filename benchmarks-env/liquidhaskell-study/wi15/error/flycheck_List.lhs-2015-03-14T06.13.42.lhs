...
{-@ concatHelperMeasure                        :: List a (List a) -> List (List a) -> Int @-}
concatHelperMeasure Emp Emp = 0
concatHelperMeasure x1 Emp = size x1
concatHelperMeasure x1 ((:+:) x2 xs) = (size x1 + size x2) + (concatHelperMeasure xs Emp)
...
{-@ concatHelper :: x1: List a  -> xs: List (List a) -> ListN a {concatHelperMeasure x1 xs}  @-}
concatHelper :: List a -> List (List a) -> List a
concatHelper x1 Emp = x1
concatHelper x1  (x2 :+: xs) =  concatHelper (append x1 x2)  xs)


{-@ concat :: List (List a) -> List a @-}
concat Emp = Emp
concat (x1 :+: xs) = concatHelper x1 xs
