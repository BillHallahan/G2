...
\begin{code}
{-@ replicate :: d:Int -> a -> {v:List a | size v = d} @-}
replicate 0 _ = Emp
...
\begin{code}
{-@ concat :: vss:List (List a) -> {v:List a | size v = sumSize vss} @-}
concat Emp = Emp
concat (x :+: xs) = undefined

{-@ concat2 :: xs:List a -> ys:List a -> {v:List a | size v = (size xs) + (size ys)} @-}
concat2 Emp ys = ys
