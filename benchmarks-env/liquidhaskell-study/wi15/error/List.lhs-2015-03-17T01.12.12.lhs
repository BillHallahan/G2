...
\begin{code}
{-@ map :: (a -> b) -> as:List a -> {bs:List b | size as = size bs} @-}
map f Emp        = Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {as:List a | size as > 0} -> a  @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> as:List a -> {bs:List b | size bs = size as} -> {cs:List c | size cs = size as} @-}
...
\begin{code}
{-@ concat :: List (List a) -> List a @-}
concat (x:Emp) = singleton x
concat lol = foldr (++) [] lol
