...
\begin{code}
{-@ length :: v:List a -> {i:Int | i == size v} @-}
{-@ measure length @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ replicate :: v:Int -> a -> {l:List a | size l == v} @-}
replicate 0 _ = Emp
replicate n v = v :+: rest
    where
        rest = replicate (n - 1) v
\end{code}
...
\begin{code}
{-@ map :: (a -> b) -> v:List a -> {l:List b | size l == size v} @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> v1:List a -> {v2:List b | size v2 == size v1} -> 
                {v3:List c | size v3 == size v1}  @-}
...
\begin{code}
{-@ sizes :: List (List a) -> Int @-}
{-@ measure sizes @-}
sizes Emp = 0
sizes ((:+:) x xs) = length x + length xs

{-@ measure first @-}
{-@ first :: {v:List a | size v > 0} -> a @-}
first  (x :+: xs) = x

{-@ measure second @-}
{-@ second :: {v:List a | size v > 0} -> List a @-}
second (x :+: xs) = xs

{-@ predicate Sums X N = size (first X) + sizes (second X) = N @-}

{-@ prepend :: v:List a -> v2:List a -> {v3:List a | size v3 == (size v + size v2)} @-}
prepend Emp ys = ys
prepend xs Emp = xs
prepend (x :+: xs) ys = x :+: prepend xs ys

{-@ concat :: v:List (List a) -> {o:List a | Sums v (sizes v)} @-}
concat Emp = Emp
concat (x :+: xs) = prepend x (concat xs)
