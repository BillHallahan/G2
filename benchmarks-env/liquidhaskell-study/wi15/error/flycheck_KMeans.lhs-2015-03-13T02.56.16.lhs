...
nearest k n centers p = k
        where k = map keepCloser (M.toList centers)

keepCloser :: (Center, Point) -> (Center, Point) -> (Center, Point)
keepCloser (a,b) (c,d) = (a,b)

findClosest :: [(Center, Point)] -> Point -> Center
findClosest xs p = Data.List.map (\(a,b) -> a) xs 
\end{code}
