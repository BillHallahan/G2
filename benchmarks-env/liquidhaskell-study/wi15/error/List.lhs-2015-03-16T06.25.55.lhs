...
\begin{code}
{-@ length :: l:List a -> {v:Nat | v == size l} @-}
length :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ replicate :: n:Nat -> a -> {l:List a | size l == n} @-}
replicate 0 _ = Emp
...
\begin{code}
{-@ listHead   :: {l:List a | size l > 0} -> a @-}
listHead (x :+: xs)  = x
{-@ listTail   :: {l:List a | size l > 1} -> {l2:List a | size l2 > 0} @-}
listTail (x :+: xs)  = xs
{-@ measure sizeAll      :: List (List a) -> Int
    sizeAll (Emp)        = 0
    sizeAll ((:+:) x xs) = (size x) + (sizeAll xs)
 @-} 
{-@  concat :: in:(List (List a)) -> {out:List a | size out == sizeAll in} @-}
concat Emp:(List (List a)) 	= Emp:(List a)
concat (l :+: ls)  
	| (length l)  == 0 	= concat ls
	| (length l)  == 1	= (listHead l) :+: concat ls
	| otherwise		= (listHead l) :+: concat ((listTail l) :+: ls)
