...
{-@ measure sample4      :: List (List a) -> Int
    sample4 (Emp)        = 0
    sample4 ((:+:) x xs) = size x + sample4 xs
  @-}



{-@ concatHelper :: x1: List a -> xs: List (List a) -> {res:List a | size x1 + sample4 xs = size res} @-} 
concatHelper :: List a -> List (List a) -> List a
concatHelper x1 Emp = x1
concatHelper x1 (x2 :+: xs) = append x2 x1

{-@ concat :: xs:List (List a) -> {res:List a |  sample4 xs + sample x1 = } @-}
concat Emp = Emp
concat (x1 :+: Emp) =  x1
