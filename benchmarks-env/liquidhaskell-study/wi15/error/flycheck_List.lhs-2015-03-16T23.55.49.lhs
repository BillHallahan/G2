...
{-@ invariant {v:List a | 0 <= sizes v} @-}

{-@ join :: x:List a -> y:List a -> {z:List a | size z = (size x + size y)} @-}
join :: List a -> List a -> List a
join Emp r    = r
join ((:+:) x xs) y = join xs (x :+: y)

{-@ concat :: u:List (List a) -> {v:List a | sizes u = size v} @-}
concat Emp = Emp
concat (x :+: xs) = x `join` (concat xs)
