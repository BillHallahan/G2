...
{-@ measure nonEmptyXs       :: List (List a) -> Bool
    noEmptyXs (Emp)          = False
    noEmptyXs ((:+:) xs xss) = if size xs < 1 then (noEmptyXs xss) else True 
  @-}
    

{-@ concat                  :: xss : List (List a) -> { xs : List a | size xs = sizeXs xss && noEmptyXs xss => size xs > 0  } @-}
