...
\begin{code}
{-@ distance :: n:Nat -> p1:List Double -> ListX Double p1 -> Double @-}
...
nearest k n centers p = minKeyMap $ M.map (distance 0 p) centers

testmap = M.fromList[(0,p0), (1,p1), (2,p2)]
	where
		p0 = add 0.0 (add 0.0 empty)
		p1 = add 3.0 (add 0.0 empty)
		p2 = add 0.0 (add 3.0 empty)

testp = add 2.9 (add 1.1 empty)
\end{code}
...
\begin{code}
{-@ mergeCluster :: n:Nat -> (n1:Int, c1:Point) -> Cluster -> Cluster @-}
