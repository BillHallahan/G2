...
\begin{code}
{-@ distance :: n:Nat -> xs:Point -> PointN (size xs) -> Double @-}  
...
{-@ mergeCluster :: n:Nat -> c1:Cluster -> {v:Cluster | size (snd c1) = size (snd v)} -> {v:Cluster | size (snd c1) = size (snd v)} @-}
...
kmeans1 k n ps cs = normalize newClusters
  where
    normalize     :: M.Map a Cluster -> M.Map a PointNE
