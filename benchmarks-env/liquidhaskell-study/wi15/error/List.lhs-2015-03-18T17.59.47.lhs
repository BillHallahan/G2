...
\begin{code}
{-@ assume length :: ls:(ListN a k) -> {v:Int | v = size ls} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ replicate :: n:Int -> a -> {ls:List a | size ls == n} @-}
replicate n x = 
         if n == 0 
         then Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {ls:List a | size ls>0} -> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> la:List a -> {lb:List b | size la == size lb }-> {lc:List c | size la == size lc} @-}
...
\begin{code}
{-@ measure count :: (List (List a)) -> Int
   count (Emp)   =  0
   count (ls :+: lss) = (size ls) + (count lss)
@-}

{-@ concat :: lss:(List (List a)) ->  {ls:List a| size ls == count lss} @-}
concat Emp = Emp
concat (Emp :+: lss) = concat lss
concat (ls :+: Emp) = ls
concat ((x :+: ls) :+: lss) = x :+: (concat (ls :+: lss))
