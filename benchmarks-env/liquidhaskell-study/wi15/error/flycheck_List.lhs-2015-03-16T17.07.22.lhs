...
\begin{code}
{-@ measure size      :: x:List a -> {v: Int | (nonEmpty x = True) <=> v>0 }
    size (Emp)        = 0
    size ((:+:) x xs) = 1 + size xs
  @-}
...
\begin{code}
{-@ length :: l: List a -> {v:Int | v = size l}   @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> l1:List a -> ListX b l1 -> ListX c l1 @-}
...
\begin{code}
{-@ measure nonemptyLL :: List (List a) -> Bool @-}
                     
nonemptyLL (Emp)         = False
nonemptyLL ((x:+:_):+:_) = True
nonemptyLL (x:+:xs)      = nonemptyLL xs


{-@ measure consize :: List (List a) -> Int  
  consize (Emp)       = 0
  consize ((:+:)x xs) = size x + consize xs
@-}

{-@ measure nonEmpty :: x:List a -> {v:Bool | Prop v <=> size x > 0} @-}
nonEmpty (Emp) = False
nonEmpty ((:+:) x xs) = True


{-@ measure emptyL :: List a -> Int 

emptyL Emp         = 0
emptyL ((:+:) x y) = 1
@-}
{-@ invariant {v:List a | emptyL v == 1 <=> size v > 0} @-}




{-@ concat :: x:List (List a) -> {y:List a | size y == consize x  } @-}

concat Emp = die "cannot call with empty list"
concat l   = concat' l

--concat ((hd :+: Emp):+: Emp) = hd :+: Emp
--concat (ls  :+: Emp)        = ls
--concat (Emp :+: xs)         = concat xs
--concat ((hd :+: tl) :+: xs) = hd :+: concat (tl:+:xs)


{-@ concat' :: w:List (List a) -> {y:List a | size y == consize w &&
                                              (consize w > 0 => size y>0)} @-}


concat' (Emp)        = Emp
concat' (hd :+: tl)  = concat2 hd (concat' tl) 

--concat ls = foldr concat2 Emp ls

{-@ concat2 :: l1:List a -> l2:List a ->
                    {l3:List a | (size l3 == size l1 + size l2)
                     && (nonEmpty l1 = True => size l3 > 0)
                     && (nonEmpty l1 = True => size l3 > 0) 
                     } @-}
--concat2 Emp l2 = l2
concat2 Emp l2          = l2
concat2 (hd :+: tl)  l2 = hd :+: concat2 tl l2
