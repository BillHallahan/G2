...
{-@ measure size      :: x:(List a) -> {v:Int | noEmpty x => v >0} 
    size (Emp)        = 0
    size ((:+:) x xs) = 1 + size xs
  @-}
...
{-@ concat :: x:{l:List (List a) | consize l>0} -> {y:List a | size y == consize x  } @-}
