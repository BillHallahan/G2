...
{-@ measure sample2       ::  (List a -> Int) -> List (List a) -> List a @-}
sample2 f Emp = Emp
sample2 f (x1 :+: Emp) = f x1
sample2 f (x1 :+: (x2 :+: xs )) = sample2 f ((append x1 x2) :+: xs)
...
{-@ size2 :: (List a -> Int) -> xs:List (List a) -> {res:Int | sample2 size xs = res} @-}
size2 f Emp = 0
size2 (x1 :+: Emp) = (length x1)
size2 (x1 :+: (x2 :+: xs )) =  size2  ((append x1 x2) :+: xs)


{-@ concat :: xs:List (List a) -> {res:List a | size xs  = size res} @-}
concat Emp = Emp
concat (x1 :+: Emp) =  x1
concat (x1 :+: (x2 :+: xs )) =  concat  ((append x1 x2) :+: xs)

{-@ predicate Sum2 X N =  (sample2 size X) = size N @-}
