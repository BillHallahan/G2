...
nearest k n centers p = minKeyMap $ M.foldrWithKey (\num cor map -> M.insert num (distance n cor p) map) M.empty centers
\end{code}
...
{-@ kmeans :: Nat -> k:Nat -> n:Nat ->
              List (PointN n) -> c1:(CenteringKN k n) -> {c2:(CenteringKN k n) | (size c1) == (size c2)}
  @-}
...
\begin{code}
{-@ kmeans1 :: k:Nat -> n:Nat ->
               List (PointN n) -> c1:(CenteringKN k n) -> {c2:(CenteringKN k2 n2) | k2 == k && n2 == n && (size c1) == (size c2)}
  @-}
