...
\begin{code}
{-@ measure sizexs    :: List a -> Int
    sizexs (Emp)      = 0
    size ((:+:) xs xss) = size xs + sizexs xss
  @-}
comblength Emp              = 0
comblength (xs :+: xss)     = length xs + comblength xss

{-@ concat                  :: xss : List (List a) -> { xs : List a | size xs = comblength xss } @-}
concat Emp                  = Emp
