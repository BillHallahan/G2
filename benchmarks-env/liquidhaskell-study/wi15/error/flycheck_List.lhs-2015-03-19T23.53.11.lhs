...
\begin{code}
----@ sumSize :: x:List (List a) -> {v=Nat | size v =  --sumSize Emp = 0
---sumSize (xs :+: Emp) = length xs
---sumSize (xs :+: xss) = (length xs) + sumSize xss

{-@ concat :: (x :+: xs):List (List a) -> {v:List a| size v >= size x } @-}
concat Emp = Emp
concat (xs :+: Emp) = xs
concat (xs :+: ys :+: xss) = concat ((append xs ys) :+: xss)

{-@ append :: List a -> List a -> List a @-} ---{v=List a | length v = length x + length y} @-}
append Emp Emp = Emp
append xs Emp = xs
append Emp ys = ys
