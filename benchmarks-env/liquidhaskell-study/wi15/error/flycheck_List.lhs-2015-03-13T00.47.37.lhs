...
\begin{code}
{-@ replicate :: t:Int -> a -> {l:List a | size l = t} @-}
replicate 0 x = empty
...
\begin{code}
{-@ map :: (a -> b) -> t:List a -> {l:List b | size t = size l} @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> j:List a -> f:{k:List b | size j = size k } -> {t:List c | size t = size f} @-}
...
\begin{code}
{-@ measure length    :: List (List a) -> Int
    length (Emp)        = 0
    length ((:+:) x xs) =  size x + length xs
  @-}

{-@ concat :: v:{s:List (List a) | length s > 0} -> {l:List a | length v = size l} @-}
concat Emp         =  empty
concat (x :+: xs)  =  concatSingleLists x (concat xs)  

{-@ concatSingleLists :: x:List a -> y:List a -> {v:List a | ((size x + size y) = size v) && size v > 0} 0@-}
concatSingleLists :: List a -> List a -> List a
concatSingleLists x Emp        = x
concatSingleLists x (y :+: ys) = add y (concatSingleLists x ys)    
