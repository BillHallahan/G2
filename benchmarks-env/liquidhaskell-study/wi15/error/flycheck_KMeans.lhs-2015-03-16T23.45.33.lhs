...
nearest k n centers p = bestK
  where
    dists = M.map (\x -> distance n p x) centers
    bestK = minKeyList $ M.toList $ dists
...
\begin{code}
{-@ measure sameSzCluster :: Cluster -> Cluster -> Bool
    sameSzCluster (Cluster (n1, p1)) (Cluster (n2, p2)) = size p1 = size p2
  @-}

{-@ mergeCluster :: n : Nat -> c1 : Cluster -> { c2 : Cluster | sameSizeCluster c1 c2} -> Cluster @-}
