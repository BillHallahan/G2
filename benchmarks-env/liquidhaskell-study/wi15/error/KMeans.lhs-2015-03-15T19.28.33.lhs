...
\begin{code}
{-@ distance :: n:Nat -> {p1:Point | size p1 == n} -> {p2:Point | size p2 == n} -> Double @-}
...
\begin{code}
{-@ mergeCluster :: n:Nat -> c1:Cluster -> {c2:Cluster | size (snd c1) == size (snd c2)} -> Cluster @-}
...
\begin{code}
{-@ kmeans :: Nat -> k:Nat -> n:Nat -> List (PointN n) -> CenteringKN k n -> CenteringKN k n @-}
...
kmeans1 k n ps cs = normalize newClusters
  where
    {-@ normalize :: M.Map a c1:Cluster -> M.Map a {p:Point | size p = size (snd c1)} @-}
    normalize     = M.map (\(sz, p) -> centroid n p sz)
    newClusters   = mapReduce fm fr ps
    fm p          = singleton (nearest k n cs p, (1 :: Int, p))
    {-@ fr :: c1:Cluster -> {c2:Cluster | size (snd c1) == size (snd c2)} -> Cluster @-}
