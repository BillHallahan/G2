...
{-@ type ClusterX (Pos, PointN N) = {(a,b) | a > 0 & size b = N} @-}
{-@ type Pos        = {v:Int | v > 0} @-}
\end{code}
...
\begin{code}
{-@ distance :: n:Nat -> x:Point -> {y:Point | size y = size x} -> Double @-}
...
nearest k n centers p = minKeyMap m'
  where m' = M.map (distance n p) centers
\end{code}
...
\begin{code}
{-@ mergeCluster :: n:Nat -> v:Cluster -> {u:Cluster -> Cluster @-}
