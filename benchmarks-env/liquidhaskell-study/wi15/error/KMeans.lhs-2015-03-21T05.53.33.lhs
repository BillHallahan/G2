...
\begin{code}
{-@ myMap :: k:Nat -> n:Nat -> ((CenterK k,PointN n) -> (CenterK k,{v:Double | v >= 0.0})) -> [(CenterK k,PointN n)] -> [(CenterK k,{v2:Double | v2 >= 0.0})] @-}
myMap k n f [] = []
myMap k n f (h:t) = (f h):(myMap k n f t)

{-@ nearest :: k:Nat -> n:Nat -> CenteringKN k n -> PointN n -> CenterK k @-}
nearest k n centers p = minKeyList (myMap k n (\(key,value) -> (key,(distance n p value))) (M.toList centers))
\end{code}
