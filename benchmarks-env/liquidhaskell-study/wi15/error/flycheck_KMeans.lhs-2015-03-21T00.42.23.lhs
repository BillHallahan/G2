...
distance n px py = if length px == 0 && length py == 0
                   then sqrt $ foldr (+) 0 $ zipWith (\x y -> (x-y)^2) Emp Emp
                   else if length px > 0 && length py > 0
                        then sqrt $ foldr (+) 0 $ zipWith (\x y -> (x-y)^2) px py
                        else error "Cannot find the distance between a point and nothing"
\end{code}
