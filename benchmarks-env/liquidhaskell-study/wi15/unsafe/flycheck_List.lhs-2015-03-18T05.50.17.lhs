...
\begin{code}
{-@ length :: l:List a -> {i:Int | i = size l} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> la:List a -> ListX b la -> ListX c la @-}
...
\begin{code}
-- asdfasd {-@ concat2 :: l1:List a -> l2:List a -> {l3:List a | size l3 = size l1 + size l2} @-}
concat2 Emp        ys = ys
concat2 (x :+: xs) ys = x :+: concat2 xs ys    
                        
prop_concat2_1 = lAssert (length (concat2 l2 l3) == 5)
prop_concat2_2 = lAssert (length (concat2 l2 Emp) == 2)

{-@ measure concatSize  :: List (List a) -> Int
    concatSize (Emp)        = 0
    concatSize ((:+:) l ls) = (size l) + (concatSize ls)
  @-}
                 
{-@ concat :: ls:List (List a) -> {l:List a | size l = concatSize ls} @-}
concat Emp        = Emp                        
concat (l :+: ls) = concat2 l (concat ls)

prop_concat_1 = lAssert (length (concat xss) == 0)
  where
    xss     = Emp
