...
nearest k n centers p = minKeyList (myMap (\(key,value) -> (key,(distance n p value))) (M.toList centers))
\end{code}
...
{-@ kmeans1 :: k:Nat -> n:Nat ->
               List (PointN n) -> CenteringKN k n -> CenteringKN k n
  @-}
kmeans1 k j ps cs = normalize newClusters
  where
    normalize     :: M.Map a Cluster -> M.Map a Point
    normalize     = M.map (\(sz, p) -> centroid j p sz)
    newClusters   = mapReduce fm fr ps
    fm p          = singleton (nearest k j cs p, (1 :: Int, p))
    fr wp1 wp2    = mergeCluster j wp1 wp2
\end{code}
