...
\begin{code}
{-@ expand  :: (a -> List (k, v)) -> {x:List a | size x > 0}-> {y:List (k, v) | size y > 0} @-}
...
\begin{code}
{-@ group :: (Ord k) => {x:List (k, v) | size x > 0} -> M.Map k (List v) @-}
...
\begin{code}
{-@ collapse  :: (v -> v -> v) -> M.Map k {x:(List v) | size x > 0} -> M.Map k v @-}
