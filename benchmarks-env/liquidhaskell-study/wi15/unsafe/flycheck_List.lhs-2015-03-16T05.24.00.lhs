...
size_      :: List a -> Int
size_ (Emp)        = 0
size_ ((:+:) x xs) = 1 + size_ xs

size2_      :: List (List a) -> Int
size2_ (Emp)        = 0
size2_ ((:+:) x xs) = size_ x + size2_ xs

{-@ concat :: y:List (List a) -> {x:List a | size x = size2 y && size x > 0 && size2 y > 0} @-}

--concat ((x:+:Emp) :+: Emp)  = x :+: Emp
--concat (Emp :+: xss)        = concat xss
--concat ((x :+: xs) :+: xss) = x :+: concat (xs :+: xss)                      
concat (Emp:+:xs) | size2_ xs > 0 = concat xs
concat (x:+:Emp)  | size_ x > 0   = x
concat (x:+:xs) = if (size_ x > 0) && (size2_ xs > 0)
                  then cons_list x (concat xs)
                  else concat xs
                  where
                    cons_list Emp ys | size_ ys > 0 = ys
                    cons_list (x:+:xs) ys  | size_ (x:+:xs) > 0 && size_ ys > 0  = x :+: (cons_list xs ys)
