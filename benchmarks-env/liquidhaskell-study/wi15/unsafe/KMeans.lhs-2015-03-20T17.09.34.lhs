...
\begin{code}
{-@ distance :: n:Nat -> x:Point -> PointN (size x) -> Double @-}
...
nearest k n centers p = minKeyMap $ M.map f centers
                        where f :: Point -> Double
                              f p' = distance n p' p
\end{code}
...
\begin{code}
{-@ measure pointSize :: Cluster -> Int
            pointSize (n,p) = size p
@-}
{-@ mergeCluster :: n:Nat -> c:Cluster -> ClusterN (pointSize c) -> Cluster @-}
