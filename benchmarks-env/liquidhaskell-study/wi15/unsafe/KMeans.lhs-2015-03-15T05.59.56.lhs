...
\begin{code}
{-@ distance :: n:Nat -> p1:Point -> {p2:Point | size p2 == size p1} -> Double @-}
...
\begin{code}
{-@ kmeans :: Nat -> k:{k:Nat | k > 0} -> n:{n:Nat | n > 0} -> {l:List (PointN n) | size l > 0} -> CenteringKN k n -> CenteringKN k n
  @-}
...
kmeans1 k n ps cs = normalize newClusters
  where
    {-@ normalize :: M.Map a {c:Cluster | size (snd c) > 0} -> M.Map a {p:Point | size p == n} @-}
