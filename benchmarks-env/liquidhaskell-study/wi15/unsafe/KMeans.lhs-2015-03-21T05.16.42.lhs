...
nearest k n centers p = minKeyMap $ M.map (\a -> distance n a p) centers
\end{code}
...
kmeans1 k n ps cs = normalize newClusters
  where
    {-@ normalize :: M.Map (CenterK k) (ClusterN n) -> CenteringKN k n @-}
    normalize	  :: M.Map Center Cluster -> Centering
