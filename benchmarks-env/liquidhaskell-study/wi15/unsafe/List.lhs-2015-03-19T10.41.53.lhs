...
\begin{code}
{-@ replicate :: s:Nat -> a -> {l:List a | (size l) == s} @-}
replicate n x
	| (n < 0) = error "Size cannot be less than zero"
	| (n <= 0) = Emp
...
\begin{code}
{-@ map :: (a -> b) -> ll:List a -> {l:List b | (size l == size ll)} @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> ListN a 0 -> ListN b 0 -> ListN c 0 @-}
zipWith _ Emp Emp               = Emp
{-@ zipWith :: (a -> b -> c) -> la:List a -> lb:ListX b la -> ListX c lb @-}
...
\begin{code}
{-@ 	measure sizeR		:: List (List a) -> Int
	sizeR (Emp)		= 0
	sizeR ((:+:) x xs)	= (size x) + (sizeR xs)
@-}

{-@ concat :: ls:List (List a) -> {l:List a | (size l) >= 0 && (size l) == (sizeR ls)} @-}
{-@ invariant concat List (List a) -> {l:List a | (size l) > 0} @-}
concat Emp = Emp
concat (x :+: xs) = x `combine` (concat xs)

{-@ combine :: k:List a -> l:List a -> {v:List a | (size v) == size k + size l} @-}
combine :: List a -> List a -> List a
combine Emp l = l
combine (x :+: xs) l = xs `combine` (x :+: l)
