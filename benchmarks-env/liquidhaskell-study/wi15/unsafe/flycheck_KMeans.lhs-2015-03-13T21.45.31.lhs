...
nearest k n centers p = minKeyMap t
        where t = M.map (\a -> distance n a p) centers
...
{-@ mergeCluster :: n:Nat -> x:Cluster -> l:{y:Cluster | clusterSize x = clusterSize y} -> {z:Cluster | (clusterSize z = clusterSize l) && (clusterSize z = clusterSize x)}  @-}
