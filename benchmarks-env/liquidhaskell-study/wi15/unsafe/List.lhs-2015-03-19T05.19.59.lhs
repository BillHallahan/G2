...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> xs:List a -> ListN b {size xs} -> ListN c {size xs} @-}
...
{-@ measure nestedSize :: List (List a) -> Int
    nestedSize (Emp)        = 0
    nestedSize ((:+:) x xs) = size xs + nestedSize xs
  @-}

{-@ concatHelper :: xs:List a -> xd:List a -> ListN a {size xs + size xd} @-}
concatHelper Emp Emp         = Emp
concatHelper (x :+: Emp) xs' = x :+: xs'
concatHelper (x:+:xs)    xs' = x :+: concatHelper xs xs'

{-@ concat :: xs:List (List a) -> ListN a {nestedSize xs} @-}
concat (Emp :+: xs) = concat xs
concat (xs :+: xs') = concatHelper xs (concat xs')
