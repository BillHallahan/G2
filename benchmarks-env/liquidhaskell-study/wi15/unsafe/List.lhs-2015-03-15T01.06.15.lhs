...
\begin{code}
{-@ replicate :: n:Int -> a -> {l:List a | size l == n} @-}
replicate 0 _ = empty
...
\begin{code}
{-@ map :: (a -> b) -> li:List a -> {lo:List b | size lo == size li} @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> la:List a -> lb:{lb:List b | size lb == size la} -> {lc:List c | size lc == size lb} @-}
...
\begin{code}
{-@ concat :: li:List (List a) -> {l:List a | size l == nestedSize li} @-}
concat ((:+:) x xs) = foldr (+:+) x xs

{-@ (+:+) :: la:List a -> lb:List a -> {l:List a | size l == size la + size lb} @-}
(+:+) Emp ys = ys 
(+:+) (x:+:Emp) ys = x :+: ys
(+:+) (x:+:xs) ys = x :+: (xs +:+ ys)


-- nestedLenght xs = foldr (+) 0 (map (\x -> length x) xs)
nestedLenght :: List (List a) -> Int
nestedLenght Emp = 0
nestedLenght ((:+:) x xs) = length x + nestedLenght xs

{-@ measure nestedSize      :: List (List a) -> Int
    nestedSize (Emp)        = 0
    nestedSize ((:+:) x xs) = size x + nestedSize xs
  @-}
