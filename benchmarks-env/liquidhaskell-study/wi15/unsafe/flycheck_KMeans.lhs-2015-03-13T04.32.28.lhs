...
\begin{code}
{-@ nearest :: k:{x:Nat | x > 1} -> n:Nat  -> CenteringKN k n -> PointN n -> {x:CenterK k | x > 0 && x < k} @-}
nearest k n centers p 
        | x < k = x
        | otherwise = 1
        where (x,y) = findClosest (M.toList t) p
              t = M.map (\a -> distance n a p) centers

findClosest :: [(Center, Double)] -> Point -> (Center, Double)
findClosest (x:[]) p = x 
findClosest ((k,v):xs) p 
            | v < b = (k,v)
            | otherwise = (a,b)
            where (a,b) = findClosest xs p
\end{code}
...
\begin{code}
{-@ measure clusterSize  :: Cluster -> Int
    clusterSize (a,b)  =  size b
  @-}

{-@ mergeCluster :: n:Nat -> x:Cluster -> {y:Cluster | clusterSize x = clusterSize y} -> Cluster @-}
...
\begin{code}
{-@ kmeans1 :: k:Nat -> n:Nat -> List (PointN n) -> CenteringKN k n -> CenteringKN k n
  @-}
