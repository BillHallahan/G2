...
kmeans1 k n ps cs = normalize k n (newClusters k n ps)
  where
    ...
    newClusters k n ps = mapReduce (fm k n cs) (fr n) ps
    {-@ fm :: k:Nat -> n:Nat -> CenteringKN k n -> PointN n -> List (CenterK k, ClusterN n)  @-}
    fm k n c p        = singleton (nearest k n c p, (1 :: Int, p))
