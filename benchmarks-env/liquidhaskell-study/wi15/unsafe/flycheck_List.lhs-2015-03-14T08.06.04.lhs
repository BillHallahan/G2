...
{-@ measure sample2       :: (List a -> Int)  -> List (List a) -> Int @-}
sample2 f (Emp)        = 0
sample2 f (x :+: xs) =  f x + sample2 f xs
...
{-@ sum3 :: xs:List (List a) -> {vlen:Int | vlen = sample size xs } @-}
sum3 Emp =0
sum3 (x :+: xs) = length x + (sum3 xs)


{-@ concat :: xs:List (List a) -> {res:List a | Sum xs res } @-}
concat Emp = Emp
concat  xs = foldr (\ls res -> (append ls res)) Emp xs 

{-@ predicate Sum2 X N =  sample size X = size N @-}
