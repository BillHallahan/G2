...
{-@ kmeans1 :: k:Nat -> n:Nat ->
               List (PointN n) -> CenteringKN k n -> CenteringKN k n
  @-}
kmeans1 k n ps cs = M.map (\(sz, p) -> centroid n p sz)
                          (mapReduce (\p -> singleton (nearest k n cs p, (1 :: Int, p)))
                                     (mergeCluster n)
                                     ps)
  {-kmeans1 k n ps cs = normalize newClusters

  where
    {-@ normalize :: M.Map Center Cluster -> M.Map Center Point @-}
    -- normalize     :: M.Map Center Cluster -> M.Map Center Point
    normalize     = M.map (\(sz, p) -> centroid n p sz)
    -- newClusters :: M.Map (CenterK k) (CenteringKN k n)
    newClusters   = mapReduce fm fr ps
    -- fm        :: PointN n -> ListN ((Centering k), ClusterN n) 1
