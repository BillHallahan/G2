...
\begin{code}
{-@ length :: lst:List a -> {v:Int | v == size lst} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ map :: (a -> b) -> la:List a -> {lb:List b | size lb == size la} @-}
map f Emp        = Emp
...
\begin{code}
{-@ measure getTailSize :: List a -> Int
    getTailSize (Emp) = 0
    getTailSize ((:+:) x xs) = size xs
  @-}

{-@ zipWith :: (a -> b -> c) -> la:List a -> {lb:List b | getTailSize la == 0 <=> getTailSize lb == 0} -> {lc:List c | size lc == size la} @-}
zipWith _ Emp Emp               = Emp
--zipWith f (x :+: xs) (y :+: ys) = f x y :+: zipWith f xs ys
zipWith f (x :+: xs) (y :+: ys) = if length xs == 0 && length ys == 0
                                  then f x y :+: zipWith f Emp Emp
                                  else if length xs > 0 && length ys > 0
                                       then f x y :+: zipWith f xs ys
                                       else error "Cannot zip two different sizes"
...
\begin{code}
{-@ measure listOfListsSize :: List (List a) -> Int
    listOfListsSize (Emp) = 0
    listOfListsSize ((:+:) x xs) = (size x) + listOfListsSize xs
  @-}

{-@ helper :: l:List a -> ll:(List (List a)) -> {v:List a | size v == (size l + listOfListsSize ll)}  @-}
helper (x :+: Emp) Emp = x :+: Emp
helper (x :+: Emp) (l :+: ls) = x :+: (helper l ls)
helper (x :+: xs) lst = x :+: (helper xs lst)

{-@ concat :: l:(List (List a)) -> {v:List a | size v == listOfListsSize l} @-}
concat Emp = Emp
concat (x :+: xs) = (helper x xs)
