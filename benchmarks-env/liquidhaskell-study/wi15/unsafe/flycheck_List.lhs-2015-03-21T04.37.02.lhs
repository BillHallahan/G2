...
\begin{code}
{-@ length :: xs:List a -> {v:Int | (v == size xs) && (v >= 0)} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ replicate :: v:Int -> a -> {xs:List a | size xs == v} @-}
replicate n x
  | n == 0 = empty
  | otherwise = add x (replicate (n-1) x)
\end{code}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> as:List a -> ListX b as -> ListX c as @-}
...
\begin{code}
{-@ headL :: {xs:List a | size xs > 0} -> a @-}
headL :: List a -> a
headL Emp = die "Empty list has no head"
headL ((:+:) x _) = x

{-@ tailL :: xs:{List a | size xs > 0} -> {ys:List a | size ys == size xs - 1} @-}
tailL :: List a -> List a
tailL Emp = die "Emp has no tail"
tailL ((:+:) _ t) = t

{-@ lastL :: {xs:List a | size xs > 0} -> a @-}
lastL :: List a -> a
lastL Emp = die "Emp has no last element"
lastL xs
  | length xs == 1 = headL xs
  | length xs >  1 = lastL (tailL xs)

{-@ popple :: xs:{List a | size xs > 0} -> {ys:List a | size ys == size xs - 1} @-}
popple               :: List a -> List a
popple Emp           = die "Can't pop last element off Emp"
popple ((:+:) x Emp) = Emp
popple ((:+:) x xs)  = (:+:) x (popple xs)

{-@ apple :: xs:List a -> ys:List a -> {zs:List a | size zs == (size xs + size ys)} @-}
apple         :: List a -> List a -> List a
apple Emp Emp = Emp
apple Emp l   = l
apple l   Emp = l
apple l1  l2  = apple (popple l1) ((:+:) (headL l1) l2)

{-@ measure llsize     :: List (List a) -> Int @-}
llsize                 :: List (List a) -> Int
llsize Emp             = 0
llsize ((:+:) Emp xss) = llsize xss
llsize ((:+:) xs  Emp) = length xs
llsize ((:+:) xs  xss) = (length xs) + (llsize xss)

{-@ concat :: xss:List (List a) -> {xs:List a | size xs == llsize xss} @-}
concat ((:+:) Emp xss) = concat xss
concat ((:+:) xs  Emp) = xs
concat ((:+:) xs  xss) = concat ((:+:) (apple xs (headL xss)) (tailL xss))
