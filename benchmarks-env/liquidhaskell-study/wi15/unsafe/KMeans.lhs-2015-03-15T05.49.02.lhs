...
\begin{code}
{-@ kmeans :: Nat -> k:Nat -> n:Nat -> {l:List (PointN n) | size l > 0} -> CenteringKN k n -> CenteringKN k n
  @-}
...
\begin{code}
{-@ kmeans1 :: k:Nat -> n:Nat -> {l:List (PointN n) | size l > 0} -> CenteringKN k n -> CenteringKN k n
  @-}
kmeans1 k n ps cs = normalize newClusters
  where
    {-@ normalize     :: M.Map a {c:Cluster | size (snd c) == n} -> M.Map a {p:Point | size p == n} @-}
