...
\begin{code}
{-@ replicate :: len:Int -> a -> { lt:List a | size lt == len} @-}
replicate length a = if length == 0
                     then empty
                     else a :+: (replicate (length-1) a)
\end{code}
...
\begin{code}
{-@ map :: (a -> b) -> l1:List a -> { l2:List b | size l2 == size l1} @-}
map f Emp        = Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> { lt:List a | size lt > 0} -> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> l1:List a -> { l2:List b | size l2 == size l1} -> { l3:List c | size l3 == size l1} @-}
...
\begin{code}
{-@ combine :: l1:List a -> l2:List a -> {l3:List a | size l3 == size l1 + size l2} @-}
...
{-@ measure calculate :: List (List a) -> Int
    calculate (Emp)        = 0
    calculate ((:+:) xs ys)  = size xs + (calculate ys)
  @-}

{-@ concat :: llt:List (List a) -> {lt:List a | size lt == calculate llt } @-}

concat Emp = Emp
concat (xs :+: ys) = combine xs (concat ys)
