...
\begin{code}
--length            :: List a -> Int
--{-@ type PosOnly = {v:Int | v >= 0} @-}
{-@ length          :: la:List a -> {v:Int | v = size la} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ type NotEmpty a= {la:List a | size la > 0} @-}
{-@ foldr1 :: (a -> a -> a) -> NotEmpty a-> a @-}
...
{-@ measure ssize 	:: List (List a) -> Int 
    ssize (Emp)		= 0
    ssize ((:+:) x xs)  = (size x) + ssize(xs)
  @-}

{-@ concat :: la:List (List a) -> {lb:List a | size lb = ssize la} @-}
concat Emp = Emp
concat (l:+:lol) = mg l (concat lol)

{-@ mg :: la:List a -> lb:List a -> {lc:List a | size lc = (size la + size lb)} @-}
mg :: List a -> List a -> List a
mg b Emp = b
mg b (l:+:ls) = mg (l:+:b) ls
