...
\begin{code}
{-@ length :: l:List a -> {v:Int | size l =  v} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ replicate :: count:Nat -> a -> {output:List a | size output = count} @-}
replicate 0 elemt = Emp
replicate n elemt = elemt :+: replicate (n-1) elemt
\end{code}
...
\begin{code}
{-@ map :: (a -> b) -> input_list:List a -> {output_list: List b | size input_list = size output_list} @-}
map f Emp        = Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {input_list:List a  | size input_list /= 0} -> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> firstlist:List a -> {secondlist:List b | size firstlist = size secondlist} -> {outputlist:List c | size outputlist = size firstlist} @-}
...
\begin{code}
{-@ measure size_of_ll      :: List (List a) -> Int
size_of_ll (Emp)        = 0
size_of_ll ((:+:) x xs) = size x + size_of_ll xs
@-}
{-@ concat_2_lists :: firstlist:List a -> secondlist:List a -> {outputlist:List a | size outputlist = (size firstlist) + (size secondlist)} @-}
concat_2_lists :: List a -> List a -> List a
concat_2_lists Emp l2 = l2
concat_2_lists l1  Emp = l1
concat_2_lists (h1 :+: t1) l2 = h1 :+: (concat_2_lists t1 l2)
{-@ concat :: input:List (List a) -> {output:List a | size output = size_of_ll input} @-}
concat Emp = Emp
concat (h:+:t) = concat_2_lists h (concat t)
