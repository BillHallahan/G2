...
{-@ length :: x:List _ -> {l:Int | l == size x} @-}
length            :: List a -> Int
length Emp        = 0
...
{-@ concat :: xss:List (List a) -> {zs:List a | size zs == ssize xss} @-}
concat Emp = Emp
concat (xs:+:xss) = pair_concat xs (concat xss)

{-@ pair_concat :: xs:List a -> ys:List a -> {zs:List a | size zs == size xs + size ys}  @-}
pair_concat Emp ys = ys
pair_concat (x:+:xs) ys = x :+: pair_concat xs ys
