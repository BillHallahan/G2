...
\begin{code}
{-@ length :: l:List a -> {n:Nat | size l == n} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ map :: (a -> b) -> la:List a -> {lb:List b | size la == size lb} @-}
map f Emp        = Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {l:List a | size l >= 1} -> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> la:List a -> {lb:List b | size lb == size la}
                                          -> {lc:List c | size lc == size la} @-}
...
{-@ combine :: l1:List a -> l2:List a -> {l3:List a | size l1 + size l2 == size l3} @-}
combine Emp ys = ys
combine (x :+: xs) ys = x :+: combine xs ys

{-@ concat :: listlist:(List (List a)) -> {la:List a | listsize listlist == size la} @-}
concat (x :+: xs) = combine x (concat xs)
