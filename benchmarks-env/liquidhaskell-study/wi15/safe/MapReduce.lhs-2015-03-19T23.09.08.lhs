...
\begin{code}
{-@ group :: (Ord k) => List (k, v) -> M.Map k ({a:List v | size a > 0}) @-}
...
\begin{code}
-- if the Map is empty, then the mapping function would do nothing
{-@ collapse  :: (v -> v -> v) -> M.Map k ({a:List v | size a != 0}) -> M.Map k v @-}
