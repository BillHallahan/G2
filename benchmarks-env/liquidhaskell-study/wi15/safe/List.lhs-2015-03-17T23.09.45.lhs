...
\begin{code}
{-@ length :: as:List a -> {v:Int | v = size as} @-} 
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ map :: (a -> b) -> v:List a -> {z:List b | size z = size v} @-}
map f Emp        = Emp
...
{-@ measure flatsize      :: List (List a) -> Int
    flatsize (Emp)        = 0
    flatsize ((:+:) x xs) = size x + flatsize xs
  @-}


{-@ concat :: v:List (List a) -> {z:List a | size z = flatsize v} @-}
concat Emp = Emp
