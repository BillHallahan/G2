...
\begin{code}
length                :: List a -> Int
{-@ length            :: x:List a -> {val:Int|val = size x} @-}
length Emp        = 0
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {x:(List a) | (size x) >= 1 } -> a @-}
...
A helper function that concat 2 lists.
{-@ concat2 :: x:(List a) -> y:(List a) -> ListN a (sizeList ((:+:) x y)) @-}
\begin{code}
concat2 :: List a -> List a -> List a
{-@ concat2 :: x:(List a) -> y:(List a) -> {z:(List a)|size z == size x + size y} @-}
concat2 Emp Emp = Emp
concat2 l1  Emp = l1
concat2 Emp l2  = l2
concat2 xs (y :+: ys) = concat2 (add y xs) ys 
\end{code}
\begin{code}
{-@ concat :: xss:(List (List a)) -> ListN a (sizeList xss)@-}
concat (x :+: xs) = concat2 x (concat xs) 
