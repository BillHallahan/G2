...
\begin{code}
{-@ replicate :: n:Int -> a -> {v:List a | size v = n} @-}
...
\begin{code}
{-@ map :: (a -> b) -> s:List a -> {v:List b | size v = size s} @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> v1:List a -> {v2:List b | size v2 = size v1} -> {v3:List c | size v3 = size v1} @-}
...
\begin{code}
{-@ measure listSize      :: List (List a) -> Int
    listSize  Emp        = 0
    listSize ((:+:) x xs) = (size x) + listSize xs
  @-}

{-@ concatEle :: l:List a -> l2:List a -> {v:List a | size v = (size l + size l2)} @-}
concatEle Emp Emp = Emp
concatEle ( a :+: l1) ( b :+: l2) = a :+: (b :+: concatEle l1 l2) 

{-@ concat :: l:List (List a) -> {v:List a | size v = listSize l } @-}
concat Emp = Emp
concat (x :+: xs) = concatEle x (concat xs)
