...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> la:List a -> {lb:List b| ((size la==0) && (size lb==0)) || ((size la >0) && (size lb>0) && (size la==size lb)) } -> {lc:List c|size lc==size la} @-}
...
@-}
{-@ concat :: ls:List (List a) -> {l:List a|size l==sizel ls} @-}
concat Emp = Emp 
concat (l:+:Emp)=l 
