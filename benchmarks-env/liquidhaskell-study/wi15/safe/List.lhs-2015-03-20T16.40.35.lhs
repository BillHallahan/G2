...
\begin{code}
{-@ length :: v:List a -> {u:Int | size v == u} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ replicate :: v:Int -> a -> {u:List a | size u == v}@-}
replicate n x = case n of
  0 -> Emp
  _ -> add x (replicate (n-1) x)
\end{code}
...
\begin{code}
{-@ map :: (a -> b) -> v:List a -> {u:List b | size u == size v} @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> v:List a -> {u:List b | size u == size v}
                             -> {w:List c | size w == size v}@-}
...
\begin{code}
{-@ measure subtotal :: List (List a) -> Int
    subtotal Emp          =  0
    subtotal ((:+:) l ls) =  size l + subtotal ls
  @-}

{-@ con :: v:List a -> u:List a -> {w:List a | size v + size u = size w} @-}
con x Emp = x
con Emp x = x
con (x1 :+: xs1) ys = add x1 $ con xs1 ys

{-@ concat :: v:List (List a) -> {u:List a | size u == subtotal v} @-}
concat Emp        = Emp
concat (l :+: ls) = con l $ concat ls
