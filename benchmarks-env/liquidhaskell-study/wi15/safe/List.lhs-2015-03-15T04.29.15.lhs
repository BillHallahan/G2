...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> x:List a -> y:ListX b x -> ListX c x @-}
...
{-@ cat2:: x:List a -> y:List a -> ListN a {(size x)+(size y)} @-}
cat2 Emp y      = y
cat2 (a:+:as) y = a :+: (cat2 as y)

{-@ measure sum_size :: List (List a) -> Int
            sum_size (Emp)            = 0
            sum_size ((:+:) x xs)     = (size x) + sum_size xs
@-}

{-@ concat :: lhs:List (List a) -> ListN a (sum_size lhs) @-}
concat Emp = Emp
concat (a:+:Emp) = a
concat (a:+:b:+:z) = concat ((cat2 a b):+:z)
