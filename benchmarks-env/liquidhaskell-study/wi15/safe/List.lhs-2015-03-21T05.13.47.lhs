...
\begin{code}
{-@ length :: lst:List a -> {v:Int | size lst == v}  @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ replicate :: n:Int -> a -> {lst:List a|size lst == n} @-}
replicate  0 x = Emp
...
\begin{code}
{-@ map :: (a -> b) -> lst1:List a -> {lst2:List b | size lst1 == size lst2} @-}
map f Emp        = Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {lst:List a | size lst > 0} -> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> lst1:List a -> {lst2 : List b | size lst1 == size lst2} -> {lst3:List c | size lst3 == size lst1} @-}
zipWith _ Emp Emp               = Emp
zipWith f (x :+: xs) (y :+: ys) = f x y :+: zipWith f xs ys
zipWith f _          _          = die  "Bad call to zipWith"

{-@ prop_zipWith :: Num a => l:List a -> TRUE @-}
...
\begin{code}
{-@ measure sizellst      :: List (List a) -> Int
    sizellst (Emp)=0
    sizellst ((:+:) l ll) = (size l) + (sizellst ll)
@-}
{-@ concat :: llst:List (List a) -> {lst:List a | size lst == sizellst llst} @-}
concat Emp = Emp 
concat (l:+:Emp) = l
concat (Emp:+:r) = concat r
concat ((e:+:l):+:ll) =(e:+:(concat (l:+:ll))) 
