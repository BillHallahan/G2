...
{-@ concat :: l1:List (List a) -> {l2:List a | (size l2) == (sizeOfConcat l1)} @-}
concat Emp = Emp
concat (x :+: Emp) = x
concat (x :+: y :+: tail) =  concat $ (concat2flip (reverseList x) y) :+: tail

{-@ concat2flip :: l1:List a -> l2:List a -> {l3:List a |(size l3) == ((size l2) + (size l1))} @-}
concat2flip Emp Emp = Emp
concat2flip Emp ys = ys
concat2flip (x :+: xs) ys  = concat2flip xs (x :+: ys)

-- This refinement is apparently not needed...
...
lengthOfConcat   :: List (List a) -> Int
lengthOfConcat (Emp)        = 0
lengthOfConcat ((:+:) x xs) = (length x) + lengthOfConcat xs
