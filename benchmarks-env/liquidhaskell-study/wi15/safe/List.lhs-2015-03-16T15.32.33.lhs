...
\begin{code}
{-@ map :: (a -> b) -> la:List a -> ListN b (size la) @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> la:List a -> ListN b (size la) -> ListN c (size la) @-}
...
{-@ measure size_lists  :: List (List a) -> Int
    size_lists (Emp)        = 0
    size_lists ((:+:) x xs) = size x + size_lists xs
  @-}
{-@ invariant {v:List (List a) | 0 <= size_lists v} @-}

{-@ concat2 :: la:List a -> lb:List a -> ListN a {(size la) + (size lb)} @-}
concat2 (la1 :+: lar) lb = la1 :+: concat2 lar lb
concat2 Emp lb = lb

{-@ concat :: lsts:(List (List a)) -> ListN a {size_lists lsts} @-}
concat (la :+: (lb :+: lr)) = concat ((concat2 la lb) :+: lr)
concat (la :+: Emp) = la
concat (Emp) = Emp
