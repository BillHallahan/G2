...
{-@ type NEList a = {l:List a | size l > 0 } @-} 
{-@ type OEList a = {l:List a | size l == 1 } @-} 
...
-- Logic: for every k, there is at least one v, because every v comes in with a k in a pair (k, v) 

{-@ group :: (Ord k) => List (k, v) -> M.Map k (NEList v) @-}
...
\begin{code}
{-@ collapse  :: (v -> v -> v) -> M.Map k (NEList v) -> M.Map k v @-}
