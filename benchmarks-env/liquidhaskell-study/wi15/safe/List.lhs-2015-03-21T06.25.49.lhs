...
\begin{code}
{-@ length :: l: (List a) ->  {a: Int | a = (size l)} @-}
length            :: List a -> Int
length Emp        = 0
...
replicate 0 el    = Emp
replicate size el = add el (replicate (size - 1) el)
\end{code}
...
concat Emp     = Emp
concat (h:+:t) = sumEls h (concat t)

{-@ sumEls :: l1: List a -> l2: List a -> {l3: List a | (size l3) = (size l1) + (size l2)} @-}
sumEls Emp l2     = l2
sumEls (h:+:t) l2 = h:+:(sumEls t l2)
