...
\begin{code}
{-@ type NEList a = {xs:List a | size xs > 0} @-}
...
{-@ noTail :: xs:NEList a -> {ys:List a | size ys == size xs - 1} @-}
noTail (x :+: Emp) = Emp
noTail (x :+: xs)  = x :+: noTail xs  
 
{-@ concat' :: xs:List a -> ys:List a -> {zs:List a | size xs + size ys == size zs} @-}
concat' Emp ys 	= ys
concat' xs  ys 	= concat' (noTail xs) ((lastOne xs) :+: ys)

{-@ measure sizeSum      :: List (List a) -> Int
    sizeSum (Emp)        = 0
    sizeSum ((:+:) x xs) = size x + sizeSum xs
  @-}

{-@ concat :: xs:List (List a) -> {ys:List a | sizeSum xs == size ys} @-}
concat Emp 	 	 = Emp
concat (x :+: y :+: xs)  = (concat' x y) `concat'` concat xs 
concat (x :+: Emp)	 = x
