...
\begin{code}
{-@ replicate :: i:{v : Int | 0 <= v} -> a -> ListN a i @-}
replicate i a
  | i < 0  = die "Can't replicate a negative number of times!"
  | i == 0 = Emp
  | i > 0  = a :+: replicate (i - 1) a
\end{code}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> l : List a -> ListX b l -> ListX c l @-}
...
-- measure summing up sizes of all children
{-@ measure childrenSize  :: List (List a) -> Int
    childrenSize (Emp)        = 0
    childrenSize ((:+:) l ls) = size l + childrenSize ls
  @-}

{-@ invariant {v:List (List a) | 0 <= childrenSize v} @-}

{-@ append :: l1 : List a -> l2 : List a -> {l : List a | size l == size l1 + size l2} @-}
append Emp r         = r
append l   Emp       = l
append (lh :+: lt) r = lh :+: append lt r

{-@ concat :: l1 : List (List a) -> {l : List a | size l == childrenSize l1} @-}
concat Emp        = Emp
