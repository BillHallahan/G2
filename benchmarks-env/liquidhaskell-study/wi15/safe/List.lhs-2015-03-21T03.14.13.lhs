...
\begin{code}
{-@ length :: container:List a -> {v:Int | size container == v} @-} 
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ replicate :: num:Int -> a -> ListN a {num} @-}
replicate 0 val = empty
replicate num val = add val (replicate (num-1) val)
\end{code}
...
\begin{code}
{-@ map :: (a -> b) -> container:List a -> {v: List b | size container == size v} @-}
map f Emp        = Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {container:List a | size container > 0}-> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> container:List a -> ListX b container-> ListX c container @-}
...
\begin{code}
{-@ measure sizeList :: List (List a) -> Int
    sizeList Emp = 0
    sizeList ((:+:) x xs) = size x + sizeList xs 
  @-}

{-@ concat :: container:List (List a) -> {v:List a|size v == sizeList container} @-}

concat Emp = Emp
concat (x :+: container) = x `addList` (concat container)

{-@ addList :: v1:List a -> v2:List a -> {v3:List a| size v3 == size v1 + size v2} @-}
addList Emp container = container
addList container Emp = container
addList (x :+: xs) container = x :+: (addList xs container) 
