...
\begin{code}
{-@ length :: d:List a -> {v:Int | v = size d}
  @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ map :: (a -> b) -> d:List a -> {bl:List b | size bl = size d}
    @-}
map f Emp        = Emp
...
verified by LH:
\begin{code}
{-@ type NonEmpty = {v:List a | size v /= 0} @-}
{-@ foldr1 :: (a -> a -> a) -> NonEmpty -> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> a1:List a -> {b1:List b | size a1 == size b1} -> {c1:List c | size c1 == size a1} @-}
...
\begin{code}
{-@ concat :: e:List (List a) -> {m:List a | size m == size2 e} @-}
concat (x :+: xs) = linklist x (concat xs)
concat Emp        = Emp
...
{-@ linklist :: x:List a -> y:List a -> {c:List a | size c == (size x + size y)}@-}
linklist               :: List a -> List a -> List a
linklist x (y :+: ys)  = linklist (add y x) ys
linklist x Emp = x
