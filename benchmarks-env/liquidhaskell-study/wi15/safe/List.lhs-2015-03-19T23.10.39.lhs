...
\begin{code}
{-@ length :: lis:List a  -> {n: Int | n = size lis} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ map :: (a -> b) -> lis:List a -> ListN b {size lis} @-}
map f Emp        = Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {lis:List a | size lis > 0} -> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> l1:List a -> {l2:List b | size l2 = size l1} -> ListN c {size l1} @-}
...
push Emp xs         = xs
push (x :+: xs) xs' = push xs (x :+: xs')

rev = rev' Emp

rev' xs Emp         = xs
rev' xs (x :+: xs') = rev' (x :+: xs) xs'

{-@ measure sizeD     :: List(List a) -> Int
    sizeD (Emp)        = 0
    sizeD ((:+:) x xs) = size x + sizeD xs
  @-}

{-@ concat :: deep:List (List a) -> {lis:List a | size lis = sizeD deep} @-}
concat Emp                 = Emp
concat (x :+: Emp)         = x
concat (x :+: (x' :+: xs)) = concat $ (concat' x x') :+: xs

{-@ concat' :: lis: List a -> lis2:List a -> ListN a {size lis + size lis2} @-}
concat' x = push (rev x)
