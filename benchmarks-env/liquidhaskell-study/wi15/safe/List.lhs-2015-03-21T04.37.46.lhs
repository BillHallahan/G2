...
\begin{code}
{-@ length :: v:List a -> {k:Int | k == (size v)} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ map :: (a -> b) -> t:List a -> {m: List b | size t == size m} @-}
map f Emp        = Emp
...
\begin{code}
{-@ foldr1 :: (a -> a -> a) -> {k:List a | (size k) >0} -> a @-}
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> v:List a -> {u:List b | size u == size v} -> {k:List c | size v == size k} @-}
...
\begin{code}
{-@ measure lenn      :: List (List a) -> Int
    lenn (Emp)          = 0
    lenn ((:+:) x xs) = size x + lenn xs
  @-}
{-@ concat :: o:List (List a) -> {j:List a | size j == lenn o} @-}
concat (Emp :+: Emp) = Emp
concat (Emp :+: (y :+: ys)) = concat (y :+: ys)
