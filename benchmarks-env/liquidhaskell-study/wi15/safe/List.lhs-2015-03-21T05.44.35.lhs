...
\begin{code}
{-@ map :: (a -> b) -> u:List a -> {v:List b|size u = size v }@-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> u:List a -> {v:List b|size u = size v} -> {w:List c| size u = size w} @-}
...
{-@ concat :: u:List (List a)->{v:List a|size v = listsize u } @-}
concat Emp   = Emp
concat ((:+:) Emp xs)  = concat xs
concat ((:+:) ((:+:)x xs) lists) = x :+: concat (xs :+: lists)
