...
\begin{code}
{-@ replicate :: n:Int -> a -> {ret:List a| size ret == n} @-}
replicate n x = if n == 0 then Emp
		else add x (replicate (n-1) x )

example1 = replicate 3 4
\end{code}
...
\begin{code}
{-@ map :: (a -> b) -> xs: List a -> {ret: List b | size xs == size ret} @-}
map f Emp        = Emp
...
\begin{code}
{-@ zipWith :: (a -> b -> c) -> l : List a -> rr : {r : List b | size l = size r} -> {ret : List c | size l = size ret && size ret = size rr} @-}
...
{-@ measure sizeOfNestedList :: List (List a) -> Int
sizeOfNestedList (Emp) = 0
sizeOfNestedList ( (:+:) x xs ) = size x + sizeOfNestedList xs
@-}


{-@ concat :: lofl: List (List a) -> {ret: List a | sizeOfNestedList lofl == size ret} @-}
concat  Emp 		   = Emp
