...
\begin{code}
{-@ distance :: n:Nat -> xz:Point -> {yz:Point| size xz == size yz} -> Double @-}
distance _ px py = sqrt $ foldr (+) 0 $ zipWith (\x y -> (x-y)^2) px py
\end{code}
...
-- justChecking k n centers p = minKeyMap $ M.map (distance n p) centers 
 

{-@ nearest :: k:Nat -> n:Nat -> CenteringKN k n -> PointN n -> CenterK k @-}
nearest _ n centers p = minKeyMap $ M.map (distance n p) centers  
...
mergeCluster _ (n1, p1) (n2, p2) = (n1 + n2, zipWith (+) p1 p2)
\end{code}
