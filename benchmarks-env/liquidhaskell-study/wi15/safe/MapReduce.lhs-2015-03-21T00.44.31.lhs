...
\begin{code}
{-@ type NotEmptyList v = {lv:List v | size lv > 0} @-}
{-@ mapReduce :: (a -> List (k,v)) -> (v -> v -> v) -> NotEmptyList a -> M.Map k v @-}
...
\begin{code}
{-@ group :: (Ord k) => List (k, v) -> M.Map k (NotEmptyList v) @-}
...
\begin{code}
{-@ collapse  :: (v -> v -> v) -> M.Map k (NotEmptyList v) -> M.Map k v @-}
