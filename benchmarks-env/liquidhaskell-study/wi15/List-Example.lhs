...
concat    :: Show a => List (List a) -> List a
\end{code}
</div>
...
debug_concat :: Show a => List (List a) -> List a
debug_concat xs = traceShow ("concat", xs) (concat xs)

{-@ concat                  :: xss : { xss : List (List a) | sizeXs xss > 0 } 
                            -> { xs : List a | size xs = sizeXs xss &&
                                 sizeXs xss > 0 } @-}
concat ((x :+: Emp) :+: Emp) = x :+: Emp
concat (Emp :+: xss)         = debug_concat xss
concat ((x :+: xs) :+: xss)  = x :+: debug_concat (xs :+: xss)
