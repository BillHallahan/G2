...
{-@ predicate Sum4 X N = size (fst1 X) + size (snd1 X) = N @-}
...
{-@ measure sample4       ::   List a -> List (List a) -> Int @-}
sample4 x1 Emp = length x1
sample4 x1 (x2 :+: xs) = length x1 + length x2


{-@ concatHelper :: x1: List a -> xs: List (List a) -> {res:List a | size res = sample4 x1 xs} @-} 
