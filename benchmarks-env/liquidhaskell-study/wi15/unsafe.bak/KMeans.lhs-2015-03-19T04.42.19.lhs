...
nearest k n centers p = distanceMap M.! bestDistance
    where
        centerList = M.toList centers
        distances = fmap (\(c, p') -> (distance n p' p, c)) centerList
        distanceMap = M.fromList distances
        bestDistance = minKeyMap distanceMap
\end{code}
