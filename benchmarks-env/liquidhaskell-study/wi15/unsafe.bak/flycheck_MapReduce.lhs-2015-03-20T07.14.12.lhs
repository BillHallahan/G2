...
\begin{code}
{-@ expand  :: (a -> {xs:List (k, v) | size xs > 0}) -> {ys:List a | size ys > 0} -> List (k, v) @-}
...
\begin{code}
{-@ group :: (Ord k) => {xs:List (k, v) | size xs > 0} -> M.Map k ({ys:List v | size ys > 0}) @-}
...
\begin{code}
{-@ collapse  :: (v -> v -> v) -> M.Map k ({xs:List v | size xs > 0}) -> M.Map k v @-}
