...
\begin{code}
{-@ kmeans1 :: k:Nat -> n:Nat ->
               List (PointN n) -> {CenteringKN k n | n >= 0} -> CenteringKN k n
  @-}

kmeans1 k n ps cs = normalize k n (newClusters k n ps)
  where
    {-@ normalize :: k:Nat -> n:Nat -> M.Map a (ClusterN n) -> M.Map a (PointN n) @-}
    normalize      :: Int -> Int -> M.Map a Cluster -> M.Map a Point
    normalize k n      = M.map (\(sz, p) -> centroid n p sz)
    {-@ newClusters :: k:Nat -> n:Nat -> List (PointN n) -> M.Map (CenterK k) (ClusterN n) @-}
    newClusters k n ps = mapReduce (fm k n) (fr n) ps
    {-@ fm :: k:Nat -> n:Nat -> PointN n -> List (CenterK k, ClusterN n)  @-}
    fm k n p           = singleton (nearest k n cs p, (1 :: Int, p))
    {-@ fr :: n:Nat -> ClusterN n -> ClusterN n -> ClusterN n @-}
    fr n wp1 wp2       = mergeCluster n wp1 wp2
\end{code}
