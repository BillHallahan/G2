...
\begin{code}
{-@ length :: x:List a -> {v:Nat | v = size x} @-}
length            :: List a -> Int
length Emp        = 0
...
{-@ concatHelper :: x:List a -> y:List a -> {v:List a | size v = size x + size y} @-}
concatHelper :: List a -> List a -> List a
concatHelper Emp Emp = Emp
concatHelper xs Emp = xs
concatHelper Emp ys = ys
concatHelper (x :+: xs) (y :+: ys) = x :+: concatHelper xs ys

{-@ concat :: xs:(List (List a)) -> {v:List a | size v = sumsize xs } @-}
concat Emp = Emp
concat (x :+: Emp) = x
concat (x :+: y :+: xs) = concat ((concatHelper x y) :+: xs)
