...
fuzzExecution (SB init_state bindings) = do
    ioProperty (do
        ...
        (ers, _, b, _, _) <- runG2WithConfig ["tests/TestFiles/"] ["tests/TestFiles/Fuzz.hs"] (Id (Name (T.pack "fuzz") (Just (T.pack "Fuzz")) 0 Nothing) TyUnknown) "fuzz" [] [Nothing] init_state config bindings
