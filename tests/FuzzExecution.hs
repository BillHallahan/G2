...
fuzzExecution (SB init_state bindings) = do
    ioProperty (do
        ...
        (ers, _, b, _, _, _) <- runG2WithConfig ["tests/TestFiles/"] ["tests/TestFiles/Fuzz.hs"] (Id (Name (T.pack "fuzz") (Just (T.pack "Fuzz")) 0 Nothing) TyUnknown) "fuzz" [] [Nothing] init_state config bindings
        ...
        mr <- runGhc (Just libdir) (do
                and <$> mapM (\er -> do
                                    ...
                                    case E.lookup nameCall (expr_env s) of
                                        Just e -> do
                                                    let stmt = T.unpack $ "let call " <> " = (" <> printHaskellDirtyPG pg e <> ") :: " <> mkTypeHaskellPG pg (typeOf (tyvar_env s) e)
