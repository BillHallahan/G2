...
runG2 red hal ord con pns (is@State { type_env = tenv
    ...
    printArgs True "final bindings input_names" (show (input_names bindings'''))
    ...
    let is2 = catMaybes (map (E.getIdFromName (expr_env (head ident_states''))) (input_names bindings'''))
    printArgs True "conc_args, calculated in submodel: " (show is2) 

    printArgs True "conc_args, final state, violated: " (show (subModel (head ident_states'') bindings'''))
    ...
    printArgs True "conc_args length: " (show (length (conc_args (head sm))))
...
printArgs :: Bool -> String -> String -> IO ()
printArgs b n res =
    case b of
        True -> do
            putStrLn $ "---" ++ n ++ "---"
