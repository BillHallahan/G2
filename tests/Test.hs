...
testFileWithConfig proj src m_assume m_assert m_reaches entry config = do
    ...
    let init_state = initState binds tycons cls (fmap T.pack m_assume) (fmap T.pack m_assert) (fmap T.pack m_reaches) False (isJust m_assert || isJust m_reaches) (T.pack entry) mb_modname ex
