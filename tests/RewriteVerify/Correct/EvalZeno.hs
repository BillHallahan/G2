...
"p04fin" forall n xs . walkNat n (count n (n : xs)) = walkNat n (S (count n xs))
"p05finA" forall n x xs . walkNat n (walkList xs (prop_05 n x xs)) = walkNat n (walkList xs True)
"p05finB" forall n x xs . walkNat x (walkList xs (prop_05 n x xs)) = walkNat x (walkList xs True)
"p06fin" forall n m . walkNat n (n - (n + m)) = walkNat n Z
"p07fin" forall n m . walkNat n ((n + m) - n) = walkNat n m
"p08fin" forall k m n . walkNat k ((k + m) - (k + n)) = walkNat k (m - n)
"p10fin" forall m .  walkNat m (m - m) = walkNat m Z
...
"p16fin" forall x xs . walkNat x (prop_16 x xs) = walkNat x True
"p18fin" forall i m . walkNat i (prop_18 i m) = walkNat i True
"p20fin" forall xs . walkNatList xs (len (sort xs)) = walkNatList xs (len xs)
"p21fin" forall n m . walkNat n (prop_21 n m) = walkNat n True
"p24fin" forall a b . walkNat a ((max a b) === a) = walkNat a (b <= a)
"p25fin" forall a b . walkNat b ((max a b) === b) = walkNat b (a <= b)
...
"p27fin" forall x xs ys . walkNat x (walkList xs (walkList ys (prop_27 x xs ys))) = walkNat x $ walkList xs $ walkList ys True
"p28fin" forall x xs . walkNat x (walkList xs (prop_28 x xs)) = walkNat x $ walkList xs True
"p29fin" forall x xs . walkNat x (walkList xs (prop_29 x xs)) = walkNat x $ walkList xs True
"p30fin" forall x xs . walkNat x (walkList xs (prop_30 x xs)) = walkNat x $ walkList xs True
"p37finA" forall x xs . walkNatList xs (prop_37 x xs) = walkNatList xs True
"p37finB" forall x xs . walkNat x (walkList xs (prop_37 x xs)) = walkNat x $ walkList xs True
"p38fin" forall n xs . walkNat n (walkList xs (count n (xs ++ [n]))) = walkNat n $ walkList xs $ S (count n xs)
"p48fin" forall xs . walkNatList xs (prop_48 xs) = walkNatList xs True
"p52fin" forall n xs . walkNatList xs (count n xs) = walkNatList xs (count n (rev xs))
"p53fin" forall n xs . walkNatList xs (count n xs) = walkNatList xs (count n (sort xs))
"p54fin" forall n m . walkNat n ((m + n) - n) = walkNat n m
...
"p59fin" forall xs ys . walkNatList xs (prop_59 xs ys) = walkNatList xs True
"p60fin" forall xs ys . walkList xs (walkNatList ys (prop_60 xs ys)) = walkList xs $ walkNatList ys True
"p61fin" forall xs ys . walkList xs (last (xs ++ ys)) = walkList xs (lastOfTwo xs ys)
...
"p71finB" forall x y xs . walkNat y (walkNatList xs (prop_71 x y xs)) = walkNat y $ walkNatList xs True
"p75fin" forall n m xs . walkList xs (count n xs + count n [m]) = walkList xs $ count n (m : xs)
"p76finA" forall n m xs . walkNat n (walkList xs (prop_76 n m xs)) = walkNat n $ walkList xs True
"p76finB" forall n m xs . walkNat m (walkNatList xs (prop_76 n m xs)) = walkNat m $ walkNatList xs True
...
"p81finA" forall n m xs . walkNat m (take n (drop m xs)) = walkNat m (drop m (take (n + m) xs))
"p81finB" forall n m xs . walkList xs (take n (drop m xs)) = walkList xs (drop m (take (n + m) xs))
"p85finA" forall xs ys . walkList xs (prop_85 xs ys) = walkList xs True
"p85finB" forall xs ys . walkList ys (prop_85 xs ys) = walkList ys True
  #-}
