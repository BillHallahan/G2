...
{-@ filterEven :: xs:List Int -> {ys:List {x:Int | x <= size xs + x } | size ys <= size xs} @-}
...
{-@ concat :: xs:List (List a) -> {ys:List a | size ys == sumsize xs} @-}
concat :: List (List a) -> List a
concat Nil = Nil
concat (xs :+: Nil) = xs
concat (xs :+: (ys :+: xss)) = concat ((append xs ys) :+: xss)

{-@ append :: x:List a -> y:List a -> {z:List a | size z = size x + size y} @-}
