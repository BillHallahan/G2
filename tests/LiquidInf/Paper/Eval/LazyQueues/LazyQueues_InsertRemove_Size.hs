module LazyQueues (insert, remove) where
...
{-@ insert :: flen:Int -> blen:Int -> a -> FBQ a flen blen -> (Int, Int, { q:Queue a | flen + blen + 1 == frontSize q + backSize q }) @-}
insert :: Int -> Int -> a -> Queue a -> (Int, Int, Queue a)
insert flen blen e (Q f b) = makeq flen (blen + 1) f (cons e b)

{-@ remove :: flen:{ f:Int | f > 0 } -> blen:Int -> FBQ a flen blen -> (Int, Int, a, { q:Queue a | flen + blen - 1 == frontSize q + backSize q}) @-}
