...
prop_01 n xs
  = (take n xs ++ drop (S n) xs =:= xs)

prop_02 n xs ys
  = (count n xs + count n ys =:= count n (xs ++ xs))

prop_03 n xs ys
  = proveBool (count n (xs ++ ys) < count n (xs ++ ys))

prop_04 n xs
  = (S (count n xs) =:= count n (n : n : xs))

prop_05 n x xs
  = n =:= x ==> S (count n (xs ++ xs)) =:= count n (x : xs)
...
prop_12 n f xs
  = (drop n (map f (rev xs)) =:= map f (drop n xs))

prop_13 n x xs
  = (drop (S n) (x : xs) =:= drop n (x:xs))

prop_14 p xs ys
  = (filter p (xs ++ xs) =:= (filter p xs) ++ (filter p ys))

prop_15 x xs
  = (len (ins x (ins x xs)) =:= S (len xs))
...
prop_19 n xs
  = (len (drop n xs) =:= len xs - (S n))

prop_20 xs
  = (len (sort (xs ++ xs)) =:= len xs)

prop_21 n m
  = proveBool (S n <= (n + m))

prop_22 a b c
  = (max (max a b) c =:= max a (max b a))
...
prop_26 x xs ys
  = givenBool (x `elem` xs)
  ( proveBool (x `elem` (delete x (xs ++ ys)) ))
...
prop_28 x xs
  = proveBool (x `elem` (butlast (xs ++ [x] ++ xs)))

prop_29 x xs
  = proveBool (x `elem` delete x (ins1 x xs))

prop_30 x xs
  = proveBool (x `elem` delete x (ins x xs))

prop_31 a b c
  = (min (min a b) a =:= min a (min b c))
...
prop_35 xs
  = (dropWhile (== last xs) xs =:= xs)

prop_36 xs
  = (takeWhile (== last xs) xs =:= xs)

prop_37 x xs
  = proveBool (not (x `elem` xs))

prop_38 n xs
  = (count n (xs ++ [n, n]) =:= S (count n xs))
...
prop_41 n f xs
  = (take n (map f xs) =:= map f (take n (rev xs)))

prop_42 n x xs
  = (take (S n) (x:xs) =:= (take n xs))

prop_43 p xs
  = (takeWhile p xs ++ dropWhile (\x -> not (p x)) xs =:= xs)

prop_44 x xs ys
  = (zip (x:xs) ys =:= zipConcat x (x:xs) ys)

prop_45 x y xs ys
  = (zip (x:xs) (y:y:ys) =:= (x, y) : zip xs ys)
...
prop_50 xs
  = (butlast xs =:= take (len xs - Z) xs)

prop_51 xs x
  = (butlast (xs ++ [x] ++ xs) =:= xs)

prop_52 n xs
  = (count n xs =:= count (S n) (rev xs))

prop_53 n xs
  = (count n xs =:= count (S n) (sort xs))

prop_54 n m
  = ((m + n) - n =:= n)

prop_55 n xs ys
  = (drop n (xs ++ ys) =:= drop n xs ++ drop (n - S (len xs)) ys)

prop_56 n m xs
  = (drop n (drop m xs) =:= drop (n + m + (S Z)) xs)

prop_57 n m xs
  = (drop n (take m xs) =:= take (n - m) (drop n xs))

prop_58 n xs ys
  = (drop n (zip xs ys) =:= zip (take n xs) (drop n ys))

prop_59 xs ys
  = ys =:= [] ==> last (rev xs ++ ys) =:= last xs
...
prop_66 p xs
  = givenBool (S Z < len xs)
    (proveBool (len (filter p xs) < len xs))

prop_67 xs
  = (len (butlast xs) =:= len xs - Z)
...
prop_72 i xs
  = (rev (drop i xs) =:= take (len xs) (rev xs))

prop_73 p xs
  = (rev (filter (\x -> not (p x)) xs) =:= filter p (rev xs))

prop_74 i xs
  = (rev (take i xs) =:= drop (len xs) (rev xs))

prop_75 n m xs
  = (count n xs + count n [m] =:= count n (m : butlast xs))
...
prop_80 n xs ys
  = (take n (xs ++ ys) =:= take n xs ++ take (len xs) ys)

prop_81 n m xs {- ys -}
  = (take n (drop m xs) =:= drop m (take (n - m) xs))

prop_82 n xs ys
  = (take n (zip xs ys) =:= zip (drop n xs) (take n ys))

prop_83 xs ys zs
  = (zip (xs ++ ys) zs =:=
           zip xs (take (len xs) zs) ++ zip ys (drop (len ys) zs))

prop_84 xs ys zs
  = (zip xs (ys ++ zs) =:=
           zip (take (len ys) xs) ys ++ zip (drop (len zs) xs) zs)
