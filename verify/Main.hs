...
main = do
  ...
  (vr, time, b, entry_f) <- verifyFromFile proj [src] tentry simplTranslationConfig config verify_config
...
printFuncCalls config entry b exec_res = do
    mapM_ (\execr@(ExecRes { final_state = s }) -> do
        ...
        let (gen_insts, gen_adts, mvp, inp, outp, hdls) = printInputOutput pg entry b execr
            sym_gen_out = fmap (printHaskellPG pg s) $ conc_sym_gens execr

        let print_method = case print_output config of
                                True -> \ins g m i o -> ins <> g <> m <> i <> " = " <> o 
                                False -> \ins g m i _ -> ins <> g <> m <> i

        case sym_gen_out of
            S.Empty -> T.putStrLn $ print_method gen_insts gen_adts mvp inp outp
            _ -> T.putStrLn $ print_method gen_insts gen_adts mvp inp outp <> "\t| generated: " <> T.intercalate ", " (toList sym_gen_out)
