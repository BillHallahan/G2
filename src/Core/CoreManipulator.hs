...
modify :: Show a => (a -> Expr -> (Expr, a)) -> (a -> a -> a) -> Expr -> a -> (Expr, a)
modify f g e x = let (e', x') = f x e in trace ("x = " ++ show x ++ " x' = " ++ show x') modify' f g e' x'
    where 
        modify' :: Show a => (a -> Expr -> (Expr, a)) -> (a -> a -> a) -> Expr -> a -> (Expr, a)
        ...
        modify'' :: Show a => (a -> Expr -> (Expr, a)) -> (a -> a -> a) -> [(Alt, Expr)] -> a -> ([(Alt, Expr)], a)
        modify'' _ _ [] x = ([], x)
        modify'' f g ((a, e):es) x =
            let 
                (e', x') = modify f g e x
...
-}
modifyT :: Show a => (a -> Type -> (Type, a)) -> (a -> a -> a) -> Type -> a -> (Type, a)
...
modifyTypesDataCon :: Show a => (a -> Type -> (Type, a)) -> (a -> a -> a) -> DataCon -> a -> (DataCon, a)
...
modifyTsInExpr :: Show a => (Expr -> a -> Type -> (Type, a)) -> (a -> a -> a) -> Expr -> a -> (Expr, a)
modifyTsInExpr f g e x = modify (f' x f g) g e x
    where
        f' :: Show a => a -> (Expr -> a ->  Type -> (Type, a)) -> (a -> a -> a) -> a -> Expr -> (Expr, a)
        f' _ f g x v@(Var n t) =
            let
...
modifyExpr' :: Show a => (a -> Expr -> (Expr, a)) -> (a -> a -> a) -> Expr -> a -> Expr
modifyExpr' f g e x = fst . modify f g e $ x

evalExpr :: Show a => (Expr -> a) -> (a -> a -> a) -> Expr -> a -> a
evalExpr f g e x = evalExpr' (\_ e' -> f e') g e x

evalExpr' :: Show a => (a -> Expr -> a) -> (a -> a -> a) -> Expr -> a -> a
...
modifyType' :: Show a => (a -> Type -> (Type, a)) -> (a -> a -> a) -> Type -> a -> Type
modifyType' f g t x = fst . modifyT f g t $ x

evalType :: Show a => (Type -> a) -> (a -> a -> a) -> Type -> a -> a
evalType f g t x = evalType' (\_ t' -> f t') g t x

evalType' :: Show a => (a -> Type -> a) -> (a -> a -> a) -> Type -> a -> a
...
modifyTypesInExpr'' :: Show a => (Expr -> a -> Type -> (Type, a)) -> (a -> a -> a) -> Expr -> a -> Expr
modifyTypesInExpr'' f g t x = fst . modifyTsInExpr f g t $ x

evalTypesInExpr :: Show a => (Type -> a) -> (a -> a -> a) -> Expr -> a -> a
evalTypesInExpr f g e x = evalTypesInExpr' (\_ t' -> f t') g e x

evalTypesInExpr' :: Show a =>  (Expr -> Type -> a) -> (a -> a -> a) -> Expr -> a -> a
evalTypesInExpr' f g e x = evalTypesInExpr'' (\e' _ t' -> f e' t') g e x

evalTypesInExpr'' :: Show a =>  (Expr -> a -> Type -> a) -> (a -> a -> a) -> Expr -> a -> a
