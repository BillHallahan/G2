...
stepAppLam state = [bindExpr b' ae l_st]
  where App (Lam b l t) ae = curr_expr state
        b'   = freshSeededName b state
        l_st = rename b b' (state {curr_expr = l})
...
-- | Shove Alts Inwards
shoveIn :: State -> (Alt, Expr) -> (Alt, Expr)
shoveIn state (Alt (dc, params), ae) = (Alt (dc, params), Case ae as2 t2)
...
doNDef state (Alt (dc, params), aexp) = case d of
    ...
    Var f t -> [renameList p_zip (state {curr_expr = aexp, path_cons = pcs'})]
    ...
    Data md -> if (length args == length params) && (dc == md)
        then let binds = zip params' args
                 a_st = renameList p_zip (state{curr_expr=aexp,path_cons=pcs'})
             in [bindExprList binds a_st]
        else []  -- Structural matching failure.
