...
step state = case curr_expr state of
    ...
    let unApp :: Expr -> [Expr]
        ...
        doNondef (Alt (dc, params), aexp) =
          ...
          in case d of
            ...
            Var f t ->
              let params' = freshSeededNameList params state
                  zipd = zip params params'
                  pc' = [CondAlt m (Alt (dc, params')) True] ++
                        path_cons state
  ...
  -- Assumptions and assertions have two flavors: either the LHS that denotes
  -- the predicate has been applied, or it is not, and is still a lambda. We
  -- consider the latter.
  Assume cond exp -> if isVal (state {curr_expr = exp})
    then case cond of
      ...
      otherwise -> if isVal (state {curr_expr = cond})
        then [state { curr_expr = exp
                    , path_cons = [CondExt cond True] ++ path_cons state}]
        else let csts = step (state {curr_expr = cond})
             in [cst {curr_expr = Assume (curr_expr cst) exp} | cst <- csts]
    else let ests = step (state {curr_expr = exp})
         in [est {curr_expr = Assume cond (curr_expr est)} | est <- ests]

  Assert cond exp -> if isVal (state {curr_expr = exp})
    then case cond of
      Lam b c t -> [state {curr_expr = Assert (App cond exp) exp}]
      
      -- The difference between here and assert is that we fork the state,
      -- and introduce another one in which the condition is negated.
      otherwise -> if isVal (state {curr_expr = cond})
        then [ state { curr_expr = exp
                     , path_cons = [CondExt cond True] ++ path_cons state }
             , state { curr_expr = exp
                     , path_cons = [CondExt cond False] ++ path_cons state }]
        else let csts = step (state {curr_expr = cond})
             in [cst {curr_expr = Assert (curr_expr cst) exp} | cst <- csts]
    else let ests = step (state {curr_expr = exp})
         in [est {curr_expr = Assert cond (curr_expr est)} | est <- ests]
