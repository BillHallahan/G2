...
type AltFunc = DataCon -- The DataCon being pattern match on
             -> [(Name, Name, AlgDataTy)]
             -> NameGen
             -> Id -- The Case binder
...
type DefaultFunc = [(Name, Name, AlgDataTy)] -> NameGen -> Id -> (Expr, NameGen)
...
createAlgDataTyWalks :: ExprEnv -> TypeEnv -> NameGen
                     ...
                     -> AltFunc -- Maybe creates an Alt for each DC
                     -> DefaultFunc -- Creates an Expr for a Default DC
                     -> StoreGenFunc Name AlgDataTy Walkers
                     -> (ExprEnv, NameGen, Walkers)
createAlgDataTyWalks eenv tenv ng s fe fd fs  =
    let
        tenv_list = M.toList tenv

        (eenv', ng', w) = createFuncs eenv ng tenv_list M.empty
                            (createAlgDataTyWalkName s) 
                            (createAlgDataTyWalkExpr fe fd)
                            fs
    in
    (eenv', ng', w)
...
createAlgDataTyWalkExpr :: AltFunc
                        ...
                        -> AlgDataTy
                        -> (Expr, NameGen)
createAlgDataTyWalkExpr fa fd ng nm tn (AlgDataTy _ dc) =
    let        
        ...
        (defExpr, ng4) = fd nm ng3 c_bind_id
        def = Alt Default defExpr

        (alts, ng5) = createAlgDataTyWalkAlt fa dc nm ng4 c_bind_id
...
createAlgDataTyWalkAlt f (dc@(DataCon _ _ ts):dcs) nm ng i =
    let
        (arg_ids, ng2) = freshIds ts ng

        (e, ng3) = f dc nm ng2 i arg_ids
        (as, ng4) = createAlgDataTyWalkAlt f dcs nm ng3 i
    ...
    case e of
        ...
        Nothing -> (as, ng4)
createAlgDataTyWalkAlt _ (PrimCon _:_) _ _ _ = error "PrimCon in createAlgDataTyWalkAlt"
...
createDeepSeqWalks eenv tenv ng =
    createAlgDataTyWalks eenv tenv ng "walk" 
        (\dc nna ng' _ is -> createDeepSeqExpr dc nna ng' is)
        (\_ ng' i -> (Var i, ng'))
        storeWalkerFunc
