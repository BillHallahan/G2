...
type AltFunc = DataCon -- The DataCon being pattern match on
             ...
             -> [Id] -- The non datacon arguments
             -> (Maybe Expr, NameGen)

type DefaultFunc = [(Name, Name, AlgDataTy)] -> NameGen -> Id -> [Id] -> (Expr, NameGen)
...
createAlgDataTyWalks :: ExprEnv -> TypeEnv -> NameGen
                     -> (AlgDataTy -> [Type]) -- Other arguments that should be considered
...
createAlgDataTyWalkExpr :: (AlgDataTy -> [Type])
                        -> AltFunc
                        -> DefaultFunc
...
createPolyPredArgs :: AlgDataTy -> [Type]
createPolyPredArgs (AlgDataTy ns _) =  map (\n -> TyVar $ Id n TYPE) ns 
                                    ++ map (\n -> TyFun (TyVar $ Id n TYPE) (TyBool)) ns

createPolyPredAlt :: DataCon -> [(Name, Name, AlgDataTy)] -> NameGen -> Id -> [Id] -> [Id] -> (Maybe Expr, NameGen)
