...
verifyLoop solver ns lemmas states b config nc sym_ids folder_root k n | (n /= 0) || (null states) = do
  ...
  (b', k', proven, lemmas') <- verifyLoopPropLemmas solver allTactics ns lemmas b config nc k
  ...
  case pl_sr of
      ...
      ContinueWith _ pl_lemmas -> do
          (sr, b'''', k''') <- verifyLoopWithSymEx solver allTactics ns lemmas'' b''' config nc k'' states
          case sr of
...
verifyLoopPropLemmas solver tactics ns lemmas b config nc k = do
    let prop_lemmas = proposedLemmas lemmas
        verify_lemma = verifyLoopPropLemmas' solver tactics ns lemmas config nc
...
verifyLoopPropLemmas' solver tactics ns lemmas config nc
                     l@(Lemma { lemma_to_be_proven = states }) = do
    (b, k) <- CM.get
    --W.liftIO $ putStrLn $ "k = " ++ show k
    --W.liftIO $ putStrLn $ lemma_name l
    (sr, b', k') <- W.lift (verifyLoopWithSymEx solver tactics ns lemmas b config nc k states)
    CM.put (b', k')
    lem <- case sr of
...
verifyLoopWithSymEx solver tactics ns lemmas b config nc k states = do
...
verifyLemmasWithNewProvenLemmas solver nl_tactics ns proven lemmas b config nc fld_name k = do
    ...
    (b', k', new_proven, lemmas') <-
          verifyLoopPropLemmas solver tactics ns lemmas b config nc k
