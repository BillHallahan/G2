...
moreRestrictivePC solver s1 s2 hm = do
  ...
  res <- if null old_conds
         ...
         then W.liftIO $ LA.applySolver solver (P.insert neg_conj P.empty) s1 s2
         else W.liftIO $ LA.applySolver solver (P.insert neg_imp P.empty) s1 s2
  case res of
    S.UNSAT () -> return True
    _ -> return False
                           ...
                           , Left lems <- moreResFA ->
                                let
                                    ...
                                    cs (E.Sym i_) = case E.lookupConcOrSym (idName i_) h2' of
                                      Nothing -> E.Sym i_
                                      Just c -> c
                                    h2_ = A.envMerge (E.mapConcOrSym cs h2) h2'
                                ...
                                Left $ (A.mkProposedLemma "lemma" s1 s2 ls2 ls1):lems
