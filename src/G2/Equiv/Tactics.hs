        ...
        case mr_sub of
            Right hm -> do
                let v_rep = HM.toList hm
                    ...
                    lkp n s = lookupConcOrSymBoth n (expr_env s) (opp_env $ track s)
                    rhs_e' = replaceVars (A.inlineFull lkp (HS.toList ns) rhs_s $ exprExtract rhs_s) v_rep
...
lemmaSound ns s lem =
  ...
  case unApp . modifyASTs stripTicks . A.inlineFull lkp (HS.toList ns) s $ exprExtract s of
    Var (Id f _):_ ->
        let
            lem_vars = varNames $ A.inlineFull lkp (HS.toList ns) s $ exprExtract (lemma_rhs lem)
        in
        not $ f `elem` lem_vars
    _ -> False
