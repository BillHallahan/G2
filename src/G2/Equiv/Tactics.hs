...
-- TODO new functions for guarded coinduction
-- these might not be helpful
-- TODO remove redundancies with regular coinduction too
-- TODO uses exact same markers
...
guardedFoldL :: S.Solver solver =>
                ...
                (StateET, StateET) ->
                W.WriterT [Marker] IO (Either [Lemma] (Maybe (StateET, Lemma), Maybe (StateET, Lemma), PrevMatch EquivTracker))
guardedFoldL solver ns lemmas gen_lemmas (sh1, sh2) (s1, s2)  = do
  let prev = prevFull (sh1, sh2)
  res <- moreRestrictivePairWithLemmasOnFuncApps solver ns lemmas prev (s1', s2')
  case res of
    Right (_, _, pm) -> let (q1, _) = present pm
                            (p1, _) = past pm
                        in
                        if (length $ dc_path $ track q1) > (length $ dc_path $ track p1)
                        then return res
                        else backtrack []
    Left new_lems -> backtrack new_lems
  where
      ...
      backtrack new_lems_ =
          case backtrackOne sh2 of
              Nothing -> return . Left $ new_lems_ ++ gen_lemmas
              Just sh2' -> guardedFoldL solver ns lemmas
                                       (new_lems_ ++ gen_lemmas) (sh1, sh2') (s1, latest sh2')
...
tryGuarded :: S.Solver s => Tactic s
tryGuarded solver ns lemmas _ (sh1, sh2) (s1, s2) = do
  res_l <- guardedFoldL solver ns lemmas [] (sh1, sh2) (s1, s2)
  case res_l of
    Right (lem_l, lem_r, pm) -> do
      let cml = CoMarker {
        co_real_present = (s1, s2)
      ...
      return $ Success Nothing
    Left l_lemmas -> do
      res_r <- guardedFoldL solver ns lemmas [] (sh2, sh1) (s2, s1)
      case res_r of
        Right (lem_l', lem_r', pm') -> do
          let cmr = CoMarker {
            co_real_present = (s2, s1)
