...
moveOutStatePieces tenv_name s = do
    let -- s' = s { type_env = M.empty, path_conds = undefined }
        ...
        sym_func_constraints_exp = liftDataT (sym_func_constraints s)
        solving_sym_func_constraints_exp = liftDataT (solving_sym_func_constraints s)
        ...
        families_exp = liftDataT (families s)
    ...
    [| State { expr_env = $(expr_env_exp)
             ...
             , sym_func_constraints = $(sym_func_constraints_exp)
             , solving_sym_func_constraints = $(solving_sym_func_constraints_exp)
    ...
    case qqRedHaltOrd config solver simplifier of
        (SomeReducer red, SomeHalter hal, _) -> do
            let hal' = hal <~> errorHalter <~> varLookupLimitHalter 3 <~> maxOutputsHalter (Just 1)
                ord = incrAfterN 2000 (maxAdtHeightOrderer 0) <-> bucketSizeOrderer 6
