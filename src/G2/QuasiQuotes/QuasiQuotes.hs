...
executeAndSolveStates' mergeStates b s = do
    ...
    case qqRedHaltOrd config solver simplifier mergeStates of
        (SomeReducer red, SomeHalter hal, _) -> do
            ...
            (res, _) <- runG2Post (red :<~ (MergeReducer solver simplifier) :<~ mkCountAllSteps :<~ CountMerges {- :<~ LimLogger 0 0 [2,2,2,1,2,2,2,2,2,2,2,1,1,1] "a_red" -}) hal' ord solver simplifier s b mergeStates
