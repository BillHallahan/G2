...
data SMTAST = (:>=) !SMTAST !SMTAST
            ...
            | MapConcatSMT SMTName Sort !SMTAST !SMTAST
            | MapConcatISMT SMTName Sort !SMTAST !SMTAST
...
instance AST SMTAST where
    ...
    children (MapConcatSMT _ _ x y) = [x, y]
    children (MapConcatISMT _ _ x y) = [x, y]
    ...
    modifyChildren f (MapConcatSMT n1 s1 x y) = MapConcatSMT n1 s1 (f x) (f y)
    modifyChildren f (MapConcatISMT n1 s1 x y) = MapConcatISMT n1 s1 (f x) (f y)
