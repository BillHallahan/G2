...
funcArgStateTrapRed solver = (mkSimpleReducer (const ())
    ...
    where
        ...
        matchesCPG b fa_s term_s cpg =
            let
                nonRedVars s = mapMaybe (\(_, e2) -> case e2 of
                                                            Var (Id vn _) -> Just vn
                                                            _ -> Nothing) $ non_red_path_conds s
                nrv_fa = nonRedVars fa_s
                nrv_term = nonRedVars term_s
                nrv_inter = nrv_fa `L.intersect` nrv_term
            in
            matchesConcPCGuide (input_names b ++ nrv_inter) fa_s solver (cpg { l_input_ids = l_input_ids cpg ++ nrv_inter })
        -- acc s b _ = do
        --     StTr cpg xs <- SM.get
        --     let c = mkConcPCGuide s b
...
data LLTracker = LLTracker { ll_count :: Int }
