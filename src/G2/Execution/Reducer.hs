...
-- | work_func for both WorkGraph. Reduces the ExState `exS` and returns the reduceds along with the appropriate Status
runReducerMerge' :: (Eq t, Named t, Reducer r rv t, Halter h hv t, Simplifier simplifier)
                 => ExState rv hv sov t
...
switchStates ex_s b@(_, hal, _, _, pr) =
    let
        ps = processedToState pr
        ex_s' = ex_s { halter_val = updatePerStateHalt hal (halter_val ex_s) ps (state ex_s) }
    in
    if not $ discardOnStart hal (halter_val ex_s') ps (state ex_s')
        then Just (ex_s', b)
        else Nothing
