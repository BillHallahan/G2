...
newType :: TV.TyVarEnv -> NameGen -> Id -> TypeEnv -> (Type, TypeEnv, NameGen)
newType tv ng i te = 
    let  
        (tn, ng') = freshSeededString "T" ng 
        ...
        i_k = typeOf tv i
        ty = TyCon tn i_k
        (ng''', all_ids) = L.mapAccumL generateIds ng'' (argumentTypes i_k)
...
instType s b@(Bindings {name_gen = ng, input_names = ins}) = 
    let 
        is = tyVarIds $ map (\n -> E.lookup n (expr_env s)) ins
        (ng', st') = L.foldl' (instType' (tyvar_env s)) (ng, s) is
        b'  = b {name_gen = ng'}
    in
      (st', b') 
