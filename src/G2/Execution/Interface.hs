...
runExecutionToProcessed :: (MonadIO m, Orderer or sov b t) => Reducer m rv t -> Halter m hv t -> or -> State t -> Bindings -> m (Processed (State t), Bindings)
runExecutionToProcessed = runReducer

{-# INLINE runExecution #-}
runExecution :: (MonadIO m, Orderer or sov b t) => Reducer m rv t -> Halter m hv t -> or -> State t -> Bindings -> m ([State t], Bindings)
