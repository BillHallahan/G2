...
mergeCurrExpr ctxt@(Context { s1_ = (State {curr_expr = ce1}), s2_ = (State {curr_expr = ce2}) })
    ...
    , evalOrRet1 == evalOrRet2 =
        let (ctxt', ce') = mergeExprInline ctxt HS.empty HS.empty e1 e2
...
mergeVarsInline :: Named t
                => Context t -> Expr -> Expr
                -> (Context t, Expr)
mergeVarsInline ctxt@(Context {s1_ = s1, s2_ = s2, ng_ = ng, newId_ = newId}) (Var i1) (Var i2)
    | i1 == i2 = (ctxt, Var i1)
    | (idType i1 == idType i2)
    ...
    , HS.member i2 (symbolic_ids s2) =
        let syms' = HS.insert i1 (HS.delete i2 (symbolic_ids s2))
            ctxt' = ctxt { s2_ = s2 { expr_env = E.insertSymbolic (idName i1) i1 
                                                    $ E.insert (idName i2) (Var i1) (expr_env s2)
                                    , path_conds = rename (idName i2) (idName i1) (path_conds s2)
                                    , symbolic_ids = syms' } }
        in (ctxt', Var i1)
    | idType i1 == idType i2
    , HS.member i1 (symbolic_ids s1)
    , HS.member i2 (symbolic_ids s2) =
        let (newSymId, ng') = freshId (idType i1) ng
            ...
            s1' = s1 { expr_env = E.insertSymbolic (idName newSymId) newSymId 
                                        $ E.insert (idName i1) (Var newSymId) (expr_env s1)
                     , path_conds = rename (idName i1) (idName newSymId) (path_conds s1)
                     , symbolic_ids = syms1' }
            s2' = s2 { expr_env = E.insertSymbolic (idName newSymId) newSymId 
                                        $ E.insert (idName i2) (Var newSymId) (expr_env s2)
                     , path_conds = rename (idName i2) (idName newSymId) (path_conds s2)
                     , symbolic_ids = syms2' }
            
            ctxt' = ctxt { ng_ = ng', s1_ = s1', s2_ = s2'  }
