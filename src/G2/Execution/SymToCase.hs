...
concretizeSym :: [(Id, Type)] -> Maybe Coercion -> Id -> KnownValues -> TypeEnv
              -> (ExprEnv, NameGen) -> DataCon -> ((ExprEnv, NameGen), ([PathCond], Expr))
concretizeSym bi maybeC binder kv tenv (eenv, ng) dc@(DataCon _ ts _ _)
...
createCaseExpr bi maybeC binder ti kv tenv eenv ng dcs =
    let
        (newId, ng') = freshId TyLitInt ng
        eenv' = E.insertSymbolic newId eenv

        ((eenv'', ng''), dcs') = L.mapAccumL (concretizeSym bi maybeC binder kv tenv) (eenv', ng') dcs
        ...
        newSymConstraint = restrictSymVal kv 0 (toInteger $ length dcs' - 1) newId
