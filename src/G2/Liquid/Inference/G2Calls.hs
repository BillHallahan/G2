...
gatherAllowedCalls entry m lrs ghci infconfig config lhconfig = do
    ...
    let called = concatMap (\er ->
        ...
        fc_red = SomeReducer (stdRed (sharing config') solver simplifier ~> strictRed)
...
gatherReducerHalterOrderer infconfig config lhconfig solver simplifier = do
    ...
    let red = case m_logger of
                    Just logger -> logger .~> SomeReducer (gathererReducer ~> stdRed share solver simplifier ~> strictRed)
                    Nothing -> SomeReducer (gathererReducer ~> stdRed share solver simplifier ~> strictRed)
...
realCExReducerHalterOrderer infconfig config lhconfig entry modname solver simplifier  cfn = do
    ...
    let halter =      lh_max_outputs
        ...
        lh_std_red = lhRed cfn :== Finished --> stdRed share solver simplifier ~> strictRed
...
genericG2Call config solver s bindings = do
    ...
    fslb <- runG2WithSomes (SomeReducer (stdRed share solver simplifier ~> strictRed))
...
genericG2CallLogging config solver s bindings lg = do
    ...
    fslb <- runG2WithSomes (SomeReducer (prettyLogger lg ~> stdRed share solver simplifier  ~> strictRed))
                           (SomeHalter swhnfHalter)
                           (SomeOrderer nextOrderer)
                           solver simplifier PreserveAllMC s bindings
