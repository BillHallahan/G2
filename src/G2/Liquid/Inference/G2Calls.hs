...
{-# SPECIALISE
    runLHG2Inference :: (Solver solver, Simplifier simplifier)
                    ...
                    -> SomeHalter (SM.StateT PrettyGuide IO) (ExecRes LHTracker) LHTracker
                    -> SomeOrderer (SM.StateT PrettyGuide IO) (ExecRes LHTracker) LHTracker
                    -> solver
                    -> simplifier
                    -> MemConfig
                    -> Id
...
cleanupResultsInference solver simplifier config init_id bindings ers = do
    ...
    let ers7 = 
          map (\(er@(ExecRes { final_state = s }), !_) ->
                (er { final_state =
                              s {track = 
                                    mapAbstractedInfoFCs (evalPrims (expr_env s) (type_env s) (tyvar_env s) (known_values s) (type_classes s)
...
gatherAllowedCalls entry m lrs ghci infconfig config lhconfig = do
    ...
    let called = concatMap (\er ->
        ...
        fc_red = SomeReducer (stdRed config' HS.empty retReplaceSymbFuncVar solver simplifier ~> strictRed)
...
gatherReducerHalterOrderer :: (MonadIO m, Solver solver, Simplifier simplifier)
                           ...
                           -> IO ( SomeReducer (SM.StateT PrettyGuide m) [FuncCall]
                                 , SomeHalter (SM.StateT PrettyGuide m) (ExecRes [FuncCall]) [FuncCall]
                                 , SomeOrderer (SM.StateT PrettyGuide m) (ExecRes [FuncCall]) [FuncCall])
gatherReducerHalterOrderer infconfig config lhconfig solver simplifier = do
    ...
    let red = case m_logger of
                    Just logger -> logger .~> SomeReducer (gathererReducer ~> stdRed config HS.empty retReplaceSymbFuncVar solver simplifier ~> strictRed)
                    Nothing -> SomeReducer (gathererReducer ~> stdRed config HS.empty retReplaceSymbFuncVar solver simplifier ~> strictRed)
...
runLHInferenceAll infconfig config g2lhconfig func proj fp = do
    ...
    execInfStack configs newProgress (runLHInferenceCore func (head main_mod) lrs ghci)
...
inferenceReducerHalterOrderer :: (MonadIO m, MonadIO m_run, Solver solver, Simplifier simplifier)
                              ...
                              -> InfStack m ( SomeReducer (SM.StateT PrettyGuide m_run) LHTracker
                                            , SomeHalter  (SM.StateT PrettyGuide m_run) (ExecRes LHTracker) LHTracker
                                            , SomeOrderer (SM.StateT PrettyGuide m_run) (ExecRes LHTracker) LHTracker)
inferenceReducerHalterOrderer infconfig config lhconfig solver simplifier entry mb_modname cfn st = do
    ...
    let some_red = existentialInstRed :== NoProgress .-->
                ...
                SomeReducer (allCallsRed ~>
                             higherOrderCallsRed ~>
                             stdRed config HS.empty retReplaceSymbFuncVar solver simplifier ~> 
                             strictRed)
...
realCExReducerHalterOrderer infconfig config lhconfig entry modname solver simplifier  cfn = do
    ...
    let halter =      lh_max_outputs
        ...
        lh_std_red = lhRed cfn :== Finished --> stdRed config HS.empty retReplaceSymbFuncVar solver simplifier ~> strictRed
...
genericG2Call config solver s bindings = do
    ...
    fslb <- runG2WithSomes (SomeReducer (stdRed config HS.empty retReplaceSymbFuncVar solver simplifier ~> strictRed))
...
genericG2CallLogging config solver s bindings lg = do
    ...
    fslb <- runG2WithSomes (SomeReducer (prettyLogger defPrettyTrack lg ~> stdRed config HS.empty retReplaceSymbFuncVar solver simplifier  ~> strictRed))
                           (SomeHalter swhnfHalter)
                           (SomeOrderer nextOrderer)
                           noAnalysis
                           solver simplifier PreserveAllMC s bindings
