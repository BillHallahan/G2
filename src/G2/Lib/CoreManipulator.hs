...
instance Manipulatable Expr State where
    modifyG f s x =
        let
            (type_env', x1') = modifyG f (type_env s) x
            (expr_env', x2') = modifyG f (expr_env s) x
            (currExpr', x3') = modifyG f (curr_expr s) x
            (path_cons', x4') = modifyG f (path_cons s) x
            (sym_links', x5') = modifyG f (sym_links s) x
        in
        (s {type_env = type_env', expr_env = expr_env', curr_expr = currExpr', path_cons = path_cons', sym_links = sym_links'}
