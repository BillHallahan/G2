...
createTCInstDecls :: GhcMonad m => PrettyGuide -> State t -> m ()
createTCInstDecls pg (State {type_classes = tc, expr_env = eenv}) = 
    let 
        gen_tc_pairs = getGenTCPairs Nothing tc
        all_inst_decls = intercalate "\n" $ map runMkInstance gen_tc_pairs
    in
        void . runDecls $ all_inst_decls
    where
        runMkInstance :: (Name, G2.Language.Type) -> String
        runMkInstance (n, t) = T.unpack $ mkTCInstanceHaskell pg n t tc eenv

-- TODO[Jacob]: Add check for existing Eq instance before adding standalone deriving instance.
...
runCheck init_pg comp_func modN entry chAll chAny b er@(ExecRes {final_state = s, conc_args = ars, conc_out = out}) = do
    ...
    let (_, _, mvTxt, arsTxt, outTxt, _) = printInputOutput pg v b er
...
printStateOutput config entry b@(Bindings { printed_outputs = pr_outs }) mval_res exec_res@(ExecRes { final_state = s }) = do
    ...
    -- TODO: Only printing gen ADTs referened in input, but printing all instances. Try to limit eventually
    let (gen_insts, gen_adts, mvp, inp, outp, handles) = printInputOutput pg entry b exec_res
        sym_gen_out = fmap (printHaskellPG pg s) $ conc_sym_gens exec_res
        
    let print_method = \g is m i o -> g <> is <> m <> i <> " = " <> o 
