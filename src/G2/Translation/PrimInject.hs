...
primDefs' b c l unit =
              ...
              , ("smtMap#", Lam TypeL a . Lam TypeL (x TYPE)
                          ...
                          $ mkApp [Prim Map (mkTyFun [mkTyFun [TyVar a, TyVar $ x TYPE], seqTyA, seqTyX])
                                  ...
                                  , Var $ v seqTyA ])
              , ("smtMapConcat#", Lam TypeL a . Lam TypeL (x TYPE)
                                . Lam TermL (f (mkTyFun [TyVar a, seqTyX]))
                                . Lam TermL (v seqTyA)
                                $ mkApp [Prim MapConcat (mkTyFun [mkTyFun [TyVar a, seqTyX], seqTyA, seqTyX])
                                        , Var $ f (mkTyFun [TyVar a, seqTyX])
                                        , Var $ v seqTyA ])
              , ("smtMapConcatI#", Lam TypeL a . Lam TypeL (x TYPE)
                                 . Lam TermL (f (mkTyFun [TyLitInt, TyVar a, seqTyX]))
                                 . Lam TermL (v seqTyA)
                                 $ mkApp [Prim MapConcatI (mkTyFun [mkTyFun [TyLitInt, TyVar a, seqTyX], seqTyA, seqTyX])
                                         , Var $ f (mkTyFun [TyLitInt, TyVar a, seqTyX])
                                         , Var $ v seqTyA ])
              ...
              , ("force##", Prim Force (TyForAll a (TyForAll d (TyFun (TyVar a) (TyFun (TyVar d) (TyVar d))))))
