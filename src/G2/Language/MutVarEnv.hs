...
insertMvVal n i org = HM.insert n (MVInfo { mv_val_id = i, mv_origin = org })
...
instance ASTContainer MVInfo Type where
    ...
    modifyContainedASTs f (MVInfo { mv_val_id = i, mv_origin = org }) =
            MVInfo { mv_val_id = modifyContainedASTs f i, mv_origin = org }