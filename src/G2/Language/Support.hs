...
data State t = State { expr_env :: E.ExprEnv -- ^ Mapping of `Name`s to `Expr`s
                     ...
                     , tyvar_map :: TVM -- ^ Mappings between type variables and lambda bound variables. TODO: change from [] to HS
                     ...
                     , sym_gens :: SQ.Seq Name -- ^ Variable names generated by logging `SymGen`s
                     , reached_hpc :: S.HashSet (Int, T.Text) -- ^ HPC ticks reached by the state
...
-- | Interface for the TVM
type TVM = HM.HashMap Id (S.HashSet Id)

insertEmptyInTVM :: Id -> TVM -> TVM
insertEmptyInTVM i tvm = if not $ HM.member i tvm then HM.insert i S.empty tvm
                                else error "insertEmptyInTVM: inserting empty mapping for already existing tyVar"

insertLVInTVM :: Id -> Id -> TVM -> TVM
insertLVInTVM tv lv tvm = if HM.member tv tvm then HM.adjust (S.insert lv) tv tvm
                                else  error "insertLVInTVM: trying to into set of TyVar not in map"

lookupTVInTVM :: Id -> TVM -> Maybe [Id]
lookupTVInTVM tv tvm = case HM.lookup tv tvm of
                    Just ns -> Just (S.toList ns)
                    Nothing -> Nothing

removeTVFromTVM :: Id -> TVM -> TVM
removeTVFromTVM i tvm = let tvm' = HM.delete i tvm in 
            if tvm == tvm' 
            then error "removeTVFromTVM: removing nonexistent TV from TVM"  
            else tvm'
...
activeNames :: TypeEnv -> E.ExprEnv -> S.HashSet Name -> SQ.Seq Name -> S.HashSet Name
activeNames tenv eenv explored nss
    | n SQ.:< ns <- SQ.viewl nss =
        let
...
instance Named t => Named (State t) where
    ...
    rename old new s =
        State { expr_env = rename old new (expr_env s)
               ...
               , tyvar_map = rename old new (tyvar_map s)
    ...
    renames hm s =
        State { expr_env = renames hm (expr_env s)
               ...
               , tyvar_map = renames hm (tyvar_map s)
...
instance Named Frame where
    ...
    rename old new (LamFrame u i) = LamFrame u (rename old new i)
    ...
    renames hm (LamFrame u i) = LamFrame u (renames hm i)
