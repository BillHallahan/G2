...
data State t = State { expr_env :: E.ExprEnv -- ^ Mapping of `Name`s to `Expr`s
                     ...
                     , tags :: HS.HashSet Name -- ^ Allows attaching tags to a State, to identify it later
                     , sym_gens :: S.Seq Name -- ^ Variable names generated by logging `SymGen`s
                     , reached_hpc :: HS.HashSet (Unique, T.Text) -- ^ HPC ticks reached by the state
                     , reached_fc_ticks :: [FuncCall] -- ^ FCTicks reached by the state
...
data Bindings = Bindings { fixed_inputs :: [Expr]
                         ...
                         , higher_order_inst :: HS.HashSet Name -- ^ Functions to try instantiating higher order functions with
...
activeNames :: TypeEnv -> E.ExprEnv -> HS.HashSet Name -> S.Seq Name -> HS.HashSet Name
