...
type ArbValueFunc = Type -> TypeEnv -> TV.TyVarEnv -> KnownValues -> ArbValueGen -> (Expr, ArbValueGen)
...
-- the same Type.
arbValue :: Type -> TypeEnv -> TV.TyVarEnv -> KnownValues -> ArbValueGen -> (Expr, ArbValueGen)
arbValue = arbValue' getFiniteADT HM.empty

-- | Allows the generation of arbitrary values of the given type.
-- Cuts off recursive ADTs with a Prim Undefined
-- Returns a new ArbValueGen that is identical to the passed ArbValueGen
constArbValue :: Type -> TypeEnv -> TV.TyVarEnv -> KnownValues -> ArbValueGen -> (Expr, ArbValueGen)
...
-- will give a different value the next time arbValue is called with
-- the same Type.
arbValueInfinite :: Type -> TypeEnv -> TV.TyVarEnv -> KnownValues -> ArbValueGen -> (Expr, ArbValueGen)
arbValueInfinite = arbValueInfinite' cutOffVal HM.empty

arbValueInfinite' :: Int -> HM.HashMap Name Type -> Type -> TypeEnv -> TV.TyVarEnv -> KnownValues -> ArbValueGen -> (Expr, ArbValueGen)
...
arbValue' :: GetADT
          ...
          -> ArbValueGen
          -> (Expr, ArbValueGen)
arbValue' getADTF m (TyFun t t') tenv tv kv av =
    let
      (e, av') = arbValue' getADTF m t' tenv tv kv av
    in
    (Lam TermL (Id (Name "_" Nothing 0 Nothing) t) e, av')
arbValue' getADTF m t tenv tv kv av
  | TyCon n _ <- tyAppCenter t
  , ts <- tyAppArgs t =
    maybe (Prim Undefined TyBottom, av) 
          (\adt -> getADTF m tenv tv kv av adt ts)
          (HM.lookup n tenv)
arbValue' getADTF m (TyApp t1 t2) tenv tv kv av =
  let
      (e1, av') = arbValue' getADTF m t1 tenv tv kv av
      (e2, av'') = arbValue' getADTF m t2 tenv tv kv av'
  in
  (App e1 e2, av'')
arbValue' _ _ TyLitInt _ _ _ av =
    let
        i = intGen av
    in
    (Lit (LitInt i), av { intGen = i + 1 })
arbValue' _ _ TyLitFloat _ _ _ av =
    let
        f = floatGen av
    in
    (Lit (LitFloat f), av { floatGen = f + 1 })
arbValue' _ _ TyLitDouble _ _ _ av =
    let
        d = doubleGen av
    in
    (Lit (LitDouble d), av { doubleGen = d + 1 })
arbValue' _ _ TyLitRational _ _ _ av =
    let
        r = rationalGen av
    in
    (Lit (LitRational r), av { rationalGen = r + 1 })
arbValue' _ _ TyLitChar _ _ _ av =
    let
...
arbValue' getADTF m (TyVar (Id n _)) tenv tv kv av
    | Just t <- HM.lookup n m = arbValue' getADTF m t tenv tv kv av
    | Just t@(TyVar _) <- TV.deepLookupName tv n = (Prim Undefined t, av)
    | Just t <- TV.deepLookupName tv n = arbValue' getADTF m t tenv tv kv av
arbValue' _ _ t _ _ _ av = (Prim Undefined t, av)


constArbValue' :: GetADT -> HM.HashMap Name Type -> Type -> TypeEnv -> TV.TyVarEnv ->  KnownValues -> ArbValueGen -> (Expr, ArbValueGen)
constArbValue' getADTF m (TyFun t t') tenv tv kv av =
    let
      (e, _) = constArbValue' getADTF m t' tenv tv kv av
    in
    (Lam TermL (Id (Name "_" Nothing 0 Nothing) t) e, av)
constArbValue' getADTF m t tenv tv kv av
  | TyCon n _ <- tyAppCenter t
  , ts <- tyAppArgs t =
    maybe (Prim Undefined TyBottom, av) 
          (\adt -> getADTF m tenv tv kv av adt ts)
          (HM.lookup n tenv)
constArbValue' getADTF m (TyApp t1 t2) tenv tv kv av =
  let
      (e1, _) = constArbValue' getADTF m t1 tenv tv kv av
      (e2, _) = constArbValue' getADTF m t2 tenv tv kv av
  in
  (App e1 e2, av)
constArbValue' _ _ TyLitInt _ _ _ av =
    let
        i = intGen av
    in
    (Lit (LitInt i), av)
constArbValue' _ _ TyLitFloat _ _ _ av =
    let
        f = floatGen av
    in
    (Lit (LitFloat f), av)
constArbValue' _ _ TyLitDouble _ _ _ av =
    let
        d = doubleGen av
    in
    (Lit (LitDouble d), av)
constArbValue' _ _ TyLitRational _ _ _ av =
    let
        r = rationalGen av
    in
    (Lit (LitRational r), av)
constArbValue' _ _ TyLitChar _ _ _ av =
    let
    ...
    in
    (Lit (LitChar c), av)
constArbValue' getADTF m (TyVar (Id n _)) tenv tv kv av
    | Just t <- HM.lookup n m = constArbValue' getADTF m t tenv tv kv av
    | Just t@(TyVar _) <- TV.deepLookupName tv n = (Prim Undefined t, av)
    | Just t <- TV.deepLookupName tv n = arbValue' getADTF m t tenv tv kv av
constArbValue' _ _ t _ _ _ av = (Prim Undefined t, av)

type GetADT = HM.HashMap Name Type -> TypeEnv -> TV.TyVarEnv -> KnownValues -> ArbValueGen -> AlgDataTy -> [Type] -> (Expr, ArbValueGen)
...
getFiniteADT :: HM.HashMap Name Type -> TypeEnv -> TV.TyVarEnv -> KnownValues -> ArbValueGen -> AlgDataTy -> [Type] -> (Expr, ArbValueGen)
getFiniteADT m tenv tv kv av adt ts =
    let
        (e, av') = getADT cutOffVal m tenv tv kv av adt ts
    in 
    (cutOff [] e, av')
...
getADT :: Int -> HM.HashMap Name Type -> TypeEnv -> TV.TyVarEnv -> KnownValues -> ArbValueGen -> AlgDataTy -> [Type] -> (Expr, ArbValueGen)
getADT cutoff m tenv tvnv kv av adt ts 
    | [dc] <- dataCon adt
    ...
    , _:_ <- dcs =
        let
            ...
            (av', es) = mapAccumL (\av_ t -> swap $ arbValueInfinite' (cutoff - 1) m' (applyTypeHashMap m' t) tenv tvnv kv av_) av 
