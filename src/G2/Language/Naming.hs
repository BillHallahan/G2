...
-- | Similar to `nameToStr`, but converts a `Name` to a `TB.TextBuilder`.
nameToBuilder :: Name -> TB.TextBuilder
...
maybe_StrToName str
    ...
    in
    Just $ Name (T.pack n) (fmap T.pack m') (read i :: Unique) Nothing
    | otherwise = Nothing
    where
        insertParen xs@('_':_) = ')':xs
        insertParen (x:xs) = x:insertParen xs
        insertParen [] = error "maybe_StrToName: bad tuple"

-- | Converts a `TB.TextBuilder` generated by `nameToBuilder` to a `Name`.
-- Loses location information
builderToName :: TB.TextBuilder -> Name
