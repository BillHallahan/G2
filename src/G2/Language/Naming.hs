...
-- | Get the provenance of a `Name`.
nameProv :: Name -> Provenance
nameProv (Name _ _ _ pr) = pr
...
maybe_StrToName str
    ...
    in
    Just $ Name (T.pack n) (fmap T.pack m') (read i :: Unique) ProvOther
    | otherwise = Nothing
...
mkNameGen nmd =
    ...
    in
    NameGen . maximum $ 0:map (\(Name _ _ i _) -> i + 1) allNames
...
freshSeededString t = freshSeededName (Name t Nothing 0 ProvOther)

freshSeededStrings :: [T.Text] -> NameGen -> ([Name], NameGen)
freshSeededStrings t = freshSeededNames (map (\t' -> Name t' Nothing 0 ProvOther) t)

freshSeededName :: Name -> NameGen -> (Name, NameGen)
freshSeededName (Name n m _ l) (NameGen i) =
    (Name n m i' l, NameGen i')
    where !i' = i + 1
...
freshName ngen = freshSeededName seed ngen
  where
    seed = Name "fs" Nothing 0 ProvOther

freshNames :: Int -> NameGen -> ([Name], NameGen)
freshNames i ngen = freshSeededNames (replicate i (Name "fs" Nothing 0 ProvOther)) ngen

freshId :: Type -> NameGen -> (Id, NameGen)
freshId = freshSeededId (Name "fs" Nothing 0 ProvOther)
